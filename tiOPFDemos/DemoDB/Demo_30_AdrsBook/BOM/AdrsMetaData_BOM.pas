...
type
  ...
  TAdrsMetaData = class( TPerObjAbs )
  private
...
begin
  if TableExists_LookUpListName then
    gTIPerMgr.DropTable(cTableName_LookupListName);

  if TableExists_LookUpListValue then
    gTIPerMgr.DropTable(cTableName_LookupListValue);

  if TableExists_Company then
    gTIPerMgr.DropTable(cTableName_Company);

  if TableExists_Person then
    gTIPerMgr.DropTable(cTableName_Person);

  if TableExists_EAdrs then
    gTIPerMgr.DropTable(cTableName_EAdrs);

  if TableExists_Adrs then
    gTIPerMgr.DropTable(cTableName_Adrs);
