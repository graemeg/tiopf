{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  ...
  Purpose:
    There are not three strategies available for mapping objects to a relational
    database:
    ...
    In summary, you must link in one and only one of the three PAS files below.

    The default is Adrs_SrvHardCodeSQL.pas

    Adrs_SrvAutoGen.pas should work for all the example database provided (as
    long as the table name and column names match)

    Adrs_SrvSQLManager.pas will work as long as the tiSQLManager tables exist in
    the database, and they are populated with the correct queries. This can be a
    bit of a drag to setup for a new database as it is usually not possible to
    insert text into a BLOB from an SQL script. This means the SQL must be
    manually entered into the SQLManager (we will fix this down the tract by
    offering a tiSQLManager export to XML feature)
...
// ,Adrs_SrvSQLManager, tiSQLMgr_Svr  // For SQLManager SQL
   ,Adrs_SrvAutoGenSQL  // For auto generated SQL
//  ,Adrs_SrvHardCodeSQL // For hard coded SQL
  ;
