...
uses
  Classes, SysUtils, Forms, Db, TypInfo, Graphics, Controls, tiPtnVisPerObj,
  {$IFDEF DELPHI6ORABOVE} Variants, {$ENDIF} SqlTimSt;
...
type
  ...
  TTiObjectBookmark = record
    ...
    BookMarkObject: TPerObjAbs; //Current object
    BookMarkBackup: TPerObjAbs; //Backup of object in case of edit&insert mode
  end;
  ...
  private
    ...
    procedure SetObjectClass(const Value: TPerObjAbsClass);
    ...
    function GetObjectClass: TPerObjAbsClass;
  ...
  protected
    ...
    function NewRecord: TPerObjAbs;virtual;abstract;
    procedure DelRecord(oObject: TPerObjAbs);virtual;abstract;
  ...
  public
    ...
    function GetActiveItem: TPerObjAbs;virtual;
    procedure ClearFields;reintroduce;
    procedure InsertRecord(const Values: array of const);
    procedure AppendRecord(const Values: array of const);
    //procedure ObserverFieldChanged(AObject: TPerObjAbs;AFieldName: string);
    ...
    property ObjectClass: TPerObjAbsClass read GetObjectClass write SetObjectClass;
  published
  ...
  protected
    function NewRecord: TPerObjAbs;override;
    procedure DelRecord(oObject: TPerObjAbs);override;

    function GetObjectList: TPerObjList;virtual;
    procedure InternalGotoBookmark(Bookmark: pointer);override;
    function GetObjectListIndex(oObject: TPerObjAbs): integer;
  ...
  public
    property ObjectList: TPerObjList read GetObjectList write SetObjectList;
    property SortColumn;
  ...
  protected
    function GetObjectList: TPerObjList;override;
  ...
  protected
    ...
    function NewRecord: TPerObjAbs;override;
    procedure DelRecord(oObject: TPerObjAbs);override;
    function GetObjectRecord: TPerObjAbs;virtual;
    procedure SetObjectRecord(const Value: TPerObjAbs);virtual;
  public
    property oRecord: TPerObjAbs read GetObjectRecord write SetObjectRecord;
    function GetActiveItem: TPerObjAbs;override;
  end;
  ...
  protected
    procedure OpenCursor(InfoQuery: boolean);override;
    function GetObjectRecord: TPerObjAbs;override;
  public
    procedure DataEvent(Event: TDataEvent;Info: longint);override;
  published
    property DataSetField;
...
begin
  ...
  then exit;
  (AObject as TPerObjAbs).Dirty := true;
  ...
  case PropInfo^.PropType^.Kind of
    tkString, tkLString, tkWString: begin
      if Size = 0 then begin
        ...
        //(oObject as TPerObjAbs).SetNull(PropInfo^.Name);
      end else begin
...
function TTiCustomDataset.GetObjectClass: TPerObjAbsClass;
begin
...
procedure TTiCustomDataset.SetObjectClass(const Value: TPerObjAbsClass);
begin
  CheckInactive;
  FObjectClass := Value;
  if Value <> nil then begin
    if(Value.ClassInfo = nil)or not(Value.InheritsFrom(TPerObjAbs))
    then DatabaseErrorFmt('Class %s not derived from TPerObjAbs',[Value.ClassName]);
...
begin
  if not Assigned(FObjectClass) then begin
    ...
    ObjectClass := TPerObjAbsClass(GetClass(ObjectClassName));
  end;
  if not Assigned(FObjectClass)
...
begin
  ...
  if Assigned(oObject) then begin
    ...
    Result := Assigned(oObject)and Assigned(PropInfo);
    if not Result
    then DatabaseErrorFmt('Field %s.%s not found',[ObjectClassname, cFieldName], Self);
  end
  else PropInfo := nil;
end;
        ...
        if Assigned(PropInfo^.GetProc) then begin
          ...
          case PropInfo^.PropType^.Kind of
            ...
            tkClass: begin
              ...
              else if oClass.InheritsFrom(TPerObjAbs) then begin
                if oClass.InheritsFrom(TPerObjList)
                then AddFieldDef(PropInfo, ftDataSet);
                if(nDepth > 0)and oClass.InheritsFrom(TPerObjAbs) then begin
...
begin
  ...
  if Buffer = nil then begin
    ...
    then Result := true //Boolean's zijn nooit NULL (ter presentatie)
      ...
      then Result := not(ofNull in(oObject as TPerObjAbs).GetField(PropInfo)^)
      else} Result := true;
    end;
    exit;
  end;
  ...
  if Field.FieldNo < 0 then begin
    ...
    if not(Field.DataType in[ftDataSet, ftBoolean]) then begin
      ...
      then else if false {ofNull in(oObject as TPerObjAbs).GetField(PropInfo)^}
      then exit;
    end;
...
begin
  ...
  if Field.FieldNo < 0 then begin //Calculated fields
    ...
    (oObject as TPerObjAbs).Dirty := true;
    pSrc := pchar(Buffer);
    case Field.DataType of
    ...
    //if Buffer = nil
    //then(oObject as TPerObjAbs).SetNull(PropInfo^.Name);
  end;
...
function TTiCustomDataset.GetActiveItem: TPerObjAbs;
begin
...
{procedure TTiCustomDataset.ObserverFieldChanged(AObject: TPerObjAbs;AFieldName: string);
...
begin
  ...
  while Assigned(AObject)do begin
    ...
    if Assigned(AParent)and(AParent is TPerObjAbs) then begin
...
procedure TTiRecordDataset.DelRecord(oObject: TPerObjAbs);
begin
  DatabaseError('Record cannot be removed in a RecordDataset!');
end;

function TTiRecordDataset.NewRecord: TPerObjAbs;
begin
  DatabaseError('Record should be set manually!');
  Result := nil;
end;

procedure TTiRecordDataset.SetObjectRecord(const Value: TPerObjAbs);
begin
...
function TTiRecordDataset.GetObjectRecord: TPerObjAbs;
begin
  Result := FRecord;
end;
...
function TTiRecordDataset.GetActiveItem: TPerObjAbs;
begin
  Result := oRecord;
end;
...
function TTiDataset.GetObjectListIndex(oObject: TPerObjAbs): integer;
begin
  Result := - 1;
...
var oList: TPerObjList;
begin
  oList := ObjectList;
  if Assigned(oList)
  then Result := oList.Count
  else Result := - 1;
end;

procedure TTiDataset.DelRecord(oObject: TPerObjAbs);
begin
...
function TTiDataset.NewRecord: TPerObjAbs;
//Add new record
begin
...
function TTiDataset.GetObjectList: TPerObjList;
begin
  Result := FObjectList;
end;

procedure TTiDataset.SetObjectList(const Value: TPerObjList);
begin
  FObjectList := Value;
  //if Assigned(Value)
...
function TTiNestedDataset.GetObjectList: TPerObjList;
...
begin
  if Assigned(DataSetField)and not FParentDataSet.IsEmpty then begin
    ...
    then Result := nil
    else if not(Result is TPerObjList)
    then DatabaseErrorFmt('%s is not a TPerObjAbs',[DataSetField.FieldName], Self);
  end
  else Result := nil;
end;
...
function TTiNestedRecordDataset.GetObjectRecord: TPerObjAbs;
//We need a List from the parent object.. Problems because these items are
//"not present" because the list could not be read (see DoAfterOpen)
//It only occurs during opening fase of the Dataset.
begin
  if Assigned(DataSetField) then begin
    if not DataSetField.GetData(@Result)
    then Result := nil
    else if not(Result is TPerObjAbs)
    then DatabaseErrorFmt('%s is not a TPerObjAbs',[DataSetField.FieldName], Self);
  end
  else Result := nil;
end;
