...
uses
  ...
  // tiOPF units
  tiUtils, tiBaseObject, FtiFormMgrForm, tiRoundedPanel
  ;
...
//    FtbxViewMenu     : TTBXSubmenuItem ;
    ...
    procedure SetOnFindHelpFile(const Value: TOnFindHelpFileEvent);
    procedure SetFormMessage( const pMessage: string; pMessageType: TtiUserFeedbackMessageType );
    procedure SetFormErrorMessage( const pMessage: string );
    procedure SetNavigationControlsEnabled(const Value: Boolean);
   ...
   public
     ...
     function    AddAction24(const pName : string ;
                             ...
                             const pResName : string ; const pEvent : TNotifyEvent ;
                             pHelpContext : integer = 0 ) : TAction ;
     function    FindAction(const pName : string): TAction; // TODO: We can remove FindAction by exposing the private Action fields as properties
     ...
     procedure   AddContextActions(const pList: TList);
...
procedure TtiApplicationMenuSystem.DoToolButtonFormPopup(Sender: TTBCustomItem; FromLink: Boolean);
  procedure _AddFormToMenu(const pCaption: string; const pIndex: Integer);
  var
    lItem : TTBXItem ;
  begin
...
procedure TtiApplicationMenuSystem.SetOnFindHelpFile(
  const Value: TOnFindHelpFileEvent);
begin
  if FHelpRouter <> nil then
    FHelpRouter.OnFindHelpFile := Value ;
end;

procedure TtiApplicationMenuSystem.SetFormMessage(const pMessage: string; pMessageType: TtiUserFeedbackMessageType);
begin
...
procedure TtiApplicationMenuSystem.AddContextActions(const pList: TList);
var
  i : Integer ;
begin
  ClearContextActions;
  for i := 0 to pList.Count - 1 do
    AddContextAction((TObject(pList.Items[i]) as TAction) as TAction);
...
//         ( FMainMenuBar.Items[i] <> FtbxViewMenu ) and
         ...
         ( FMainMenuBar.Items[i] <> FtbxContextMenu ) then
        FMainMenuBar.Items[i].Visible := Value ;

    FActionWindowNext.Enabled     := (FormMgr.FormCount > 1)   and ( Value ) ;
    FActionWindowPrevious.Enabled := (FormMgr.FormCount > 1)   and ( Value ) ;
  finally
    DoEndUpdate(nil);
  end;
end;
...
procedure TtiApplicationMenuSystem.SetFormCaption(const Value: TCaption);
begin
...
procedure TtiApplicationMenuSystem.SetFormErrorMessage(const pMessage: string);
begin
  SetFormMessage(pMessage, tiufmtError);
end;
