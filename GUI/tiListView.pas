...
type
  ...
  TtiLVOnFilterDataEvent  = procedure( pData   : TtiObject ; var pbInclude : boolean ) of object ;
  TtiLVGetImageIndexEvent = procedure( pData   : TtiObject ; var pImageIndex : integer ) of object ;
  TtiLVEvent              = procedure( pLV : TtiCustomListView ) of object ;
  TtiLVItemEvent          = procedure( pLV : TtiCustomListView ; pData : TtiObject ; pItem : TListItem ) of object ;
  TtiLVItemEditEvent      = procedure( pLV : TtiCustomListView ; pData : TtiObject ; pItem : TListItem  ) of object ;
  TtiLVOnGetFont          = procedure( pLV : TtiCustomListView ;
                                       pCanvas : TCanvas ;
                                       pItem   : TListItem ;
                                       pData   : TtiObject ) of object ;
  ...
  TtiLVCanPerformAction   = procedure( pLV  : TtiCustomListView ; pData : TtiObject ; pItem : TListItem ; var pbCanPerformAction : boolean ) of object ;
  ...
  private
    ...
    procedure SetInfoTipType(const Value: TtiLVInfoTypeType);
    procedure DoTipInfo(Sender: TObject; Item: TListItem; var InfoTip: string);
  protected

    function    GetDefaultInfoTip(pIndex: Integer): string; virtual;
    ...
    function    ValueFromProperty( ptiListColumn : TtiListColumn ; pData : TtiObject ) : string ;
    function    ValueFromDerivedProperty( ptiListColumn : TtiListColumn ; pData : TtiObject ) : string ;
  ...
  private
    ...
    procedure SetOnItemDelete(const Value: TtiLVItemEditEvent);
    procedure SetOnItemEdit(const Value: TtiLVItemEditEvent);
    procedure SetOnItemInsert(const Value: TtiLVItemEditEvent);
    procedure SetOnItemView(const Value: TtiLVItemEditEvent);
...
procedure TtiCustomListView.PositionCursor(pData: TtiObject);
var
  i : integer ;
begin
...
procedure TtiCustomListView.SetSelectedData(const Value: TtiObject);
begin
...
function TtiCustomListView.GetDefaultInfoTip(pIndex: Integer): string;
var
  i : Integer ;
begin
  Result := '';
  ...
  begin
    ...
    Result := Result + GetCellText( pIndex, i-1 ) ;
  end;
end;
...
procedure TtiListView.SetVisibleButtons( const Value: TtiLVVisibleButtons);
begin
...
procedure TtiListView.SetButtonStyle(const Value: TLVButtonStyle);
begin
...
procedure TtiListView.SetOnItemDelete(const Value: TtiLVItemEditEvent);
begin
  FOnItemDelete := Value;
  if Assigned(FOnItemDelete) then
    FCtrlBtnPnl.OnDelete := DoDelete
...
procedure TtiListView.SetOnItemEdit(const Value: TtiLVItemEditEvent);
begin
  FOnItemEdit := Value;
  if Assigned(FOnItemEdit) then
    FCtrlBtnPnl.OnEdit := DoEdit
...
procedure TtiListView.SetOnItemInsert(const Value: TtiLVItemEditEvent);
begin
  FOnItemInsert := Value;
  if Assigned(FOnItemInsert) then
    FCtrlBtnPnl.OnNew := DoNew
...
procedure TtiListView.SetEnabled(Value: Boolean);
begin
  inherited;
  FCtrlBtnPnl.RefreshButtons ;
end;

procedure TtiListView.SetReadOnly(const Value: boolean);
begin
  inherited;
  FCtrlBtnPnl.RefreshButtons ;
end;
...
procedure TtiListView.SetOnItemView(const Value: TtiLVItemEditEvent);
begin
  FOnItemView := Value;
  if Assigned(FOnItemView) then
    FCtrlBtnPnl.OnView := DoView
...
procedure TtiCustomListView.SetInfoTipType(const Value: TtiLVInfoTypeType);
begin
  FInfoTipType := Value;
  case FInfoTipType of
  itNone    : begin
