...
type
  ...
  private
    ...
    function   GetSimplePropType( pPersistent : TtiObject ; psPropName : string ) : TtiLVTypeKind ;
  public
    constructor Create ;
    ...
    function ValidateFilter( pValue : TtiObject ) : string ;
  ...
  private
    ...
    function  DoesDataPassFilter( pValue: TtiObject ; pFilters : TList  ): boolean;
    procedure DoOnFilterData( pData : TtiObject ; var pbInclude : boolean ) ;
...
function TtiListViewPlus.DoesDataPassFilter(pValue: TtiObject ; pFilters : TList ): boolean;
  // Scan through the filters and add an ExpEval object to the list
  // for each filter.
  procedure FiltersToExpEval( pFilters, pResult : TList ; pValue : TtiObject ) ;
  var
    i : integer ;
    lExpEval : TtiExpEval ;
  begin
...
procedure TtiListViewPlus.DoOnFilterData(pData: TtiObject; var pbInclude: boolean);
begin
...
// Does this pValue ( a TtiObject descendant ) pass the filter ?
function TtiLVFilter.DoesDataPassFilter(pValue: TtiObject): boolean;
  // The property being filtered is a string
  function TestStringFilter( pValue : TtiObject ) : boolean ;
  var
    lsProp : string ;
    lsVal  : string ;
  begin
  ...
  function TestIntFilter( pValue : TtiObject ) : boolean ;
  var
    lsProp : string ;
    liProp : integer ;
    liVal  : integer ;
...
function TtiLVFilter.ValidateFilter( pValue : TtiObject ) : string;
begin
  result := '' ;
...
begin
  ...
  if ( pData = nil ) or
     ( pData.Count = 0 ) or
     ( not ( TObject(pData.Items[0]) is TtiObject )) then
    Exit ; //==>

  result := false ;
  lData := TObject( pData.Items[0]) as TtiObject;
