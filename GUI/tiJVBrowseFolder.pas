...
September-2004: Modified by Peter Hinrichsen for tiOPF to remove dependencies
                on other JVCL units.

-----------------------------------------------------------------------------}

unit tiJVBrowseFolder;
...
type
  ...
  TtiJvBrowseForFolderDialog = class(TCommonDialog, IFolderFilter, IUnknown)
  private
...
begin
  with TtiJvBrowseForFolderDialog.Create(nil) do
  try
...
var
  ...
  begin
    with CSIDLLocations[LSpecialDirectory] do
      Result := (MinVersion <= GetFileVersion('shell32.dll')) and
...
begin
  ...
  with TtiJvBrowseForFolderDialog(lpData) do
  begin
    FDialogWindow := Wnd;
    case uMsg of
      BFFM_INITIALIZED:
...
procedure TtiJvBrowseForFolderDialog.DefaultHandler(var Msg);
begin
...
function TtiJvBrowseForFolderDialog.DoGetEnumFlags(const AFolder: string;
...
procedure TtiJvBrowseForFolderDialog.DoInitialized;
const
  SBtn = 'BUTTON';
  HelpButtonId = $FFFF;
var
...
procedure TtiJvBrowseForFolderDialog.DoIUnknown(const Unknown: IUnknown);
var
...
procedure TtiJvBrowseForFolderDialog.DoSelChanged(IDList: PItemIDList);
var
...
function TtiJvBrowseForFolderDialog.DoShouldShow(
...
function TtiJvBrowseForFolderDialog.DoValidateFailed(
...
function TtiJvBrowseForFolderDialog.DoValidateFailedW(
...
function TtiJvBrowseForFolderDialog.Execute: Boolean;
var
...
begin
  ...
  if ShellVersion < $00040000 then
    raise exception.CreateRes(@RsEShellNotCompatible);
...
function TtiJvBrowseForFolderDialog.GetOwnerWindow: HWND;
var
  F: TCustomForm;
begin
...
function TtiJvBrowseForFolderDialog.GetRootDirectoryPath: string;
begin
  if FRootDirectory = fdNoSpecialFolder then
...
procedure TtiJvBrowseForFolderDialog.HookDialog;
begin
  if FDialogWindow <> 0 then
...
function TtiJvBrowseForFolderDialog.IsRootDirectoryPathStored: Boolean;
begin
  Result := (RootDirectory = fdNoSpecialFolder) and (FRootDirectoryPath > '');
end;

procedure TtiJvBrowseForFolderDialog.MainWndProc(var Msg: TMessage);
begin
  try
    Dispatch(Msg);
  except
...
procedure TtiJvBrowseForFolderDialog.SetExpanded(const APath: string);
begin
  if FDialogWindow <> 0 then
    { Implicit conversion }
    SetExpandedW(APath);
...
procedure TtiJvBrowseForFolderDialog.SetExpanded(IDList: PItemIDList);
begin
  if FDialogWindow <> 0 then
    SendMessage(FDialogWindow, BFFM_SETEXPANDED, WPARAM(False), LPARAM(IDList));
end;

procedure TtiJvBrowseForFolderDialog.SetExpandedW(const APath: WideString);
begin
  if FDialogWindow <> 0 then
...
procedure TtiJvBrowseForFolderDialog.SetOKTextW(const AText: WideString);
begin
  if FDialogWindow <> 0 then
...
procedure TtiJvBrowseForFolderDialog.SetSelection(IDList: PItemIDList);
begin
  if FDialogWindow <> 0 then
    SendMessage(FDialogWindow, BFFM_SETSELECTION, WPARAM(False), LPARAM(IDList));
end;

procedure TtiJvBrowseForFolderDialog.SetStatusText(const AText: string);
begin
  if FDialogWindow <> 0 then
    SendMessage(FDialogWindow, BFFM_SETSTATUSTEXT, 0, LPARAM(Pointer(AText)));
end;

procedure TtiJvBrowseForFolderDialog.SetStatusTextW(const AText: WideString);
begin
  if FDialogWindow <> 0 then
    SendMessage(FDialogWindow, BFFM_SETSTATUSTEXTW, 0, LPARAM(PWideChar(AText)));
end;

function TtiJvBrowseForFolderDialog.ShouldShow(psf: IShellFolder; pidlFolder,
...
procedure TtiJvBrowseForFolderDialog.UpdateStatusText(AText: string);
const
  cStatusLabel = $3743;
var
...
procedure TtiJvBrowseForFolderDialog.WMShowWindow(var Msg: TMessage);
begin
...
procedure TtiJvBrowseForFolderDialog.WMSize(var Msg: TWMSize);
var
  BtnSize: TRect;
  WindowSize: TRect;
begin
