{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  ...
  Revision history:
    ...
    August 2002, Andrew Denton, Promoted LVConfig to Public from Protected.
    June 2003, Andrew Denton, Fixed a bug where an invalid value entered into
                              a filter would raise an exception

  Purpose:
    TtiListViewPlus component to 'browse' a TList of TPersistent(s)
    Based on the TtiListView, with additional functionality to:
      * Select columns
      * Sort
      * Filter
      * Find
  ...
  ToDo:
    1. Give ability for user to save a the current column, filter and sort,
       then apply this (or any number of) view when this data is next
       displayed.
    2. Fix the cloned code where filters and finds are assigned.
    3. Rework DoesDataPassFilter so it respects user defined order of precedence
    4. Modify the export framework to accept user defined plugins.
...
type
  ...
  protected
    ...
    procedure SelectByCaption( const psCaption : string ; const piPos : integer = -1 ) ;
  ...
  private
    ...
    FsValue     : string ;
    FsField     : string ;
    function   WildcardMatch( const pSource, pPattern: String): Boolean;
    function   GetSimplePropType( pPersistent : TPersistent ; psPropName : string ) : TtiLVTypeKind ;
  public
    constructor Create ;
    ...
    property Field    : string      read FsField     write FsField ;
    function DoesDataPassFilter( pValue : TPersistent ) : boolean ;
    function ValidateFilter( pValue : TPersistent ) : string ;
  ...
  public
    ...
    procedure   Assign( const pData : TtiLVConfig ) ;
    function    IsCompatible( const pData : TList ) : boolean ;
    procedure   AssignAvailableColumns(pStrings : TStrings);
    procedure   AssignSelectedColumns(pStrings : TStrings);
  end ;
  ...
  private
    ...
    procedure ListToHTMLStream( pStream : TStream );

    procedure ListToStream( const pStream : TStream ;
    ...
    function  ValidateFilters( var psMessage : string ; pFilters : TList ) : boolean;
    function  DoesDataPassFilter( pValue: TPersistent ; pFilters : TList  ): boolean;
    procedure DoOnFilterData( pData : TPersistent ; var pbInclude : boolean ) ;

    procedure ViewFile(const psFileName: string);
    ...
    procedure WriteTextToStream(pStream: TStream; psText: string);
    function  INIReadString(const pSection, pIdent, pDefault : string): string;
    procedure INIWriteString(const pSection, pIdent, pValue : string);
...
procedure tiShowWithListView( pList : TList ; const pCols : array of string ) ;
...
procedure ShowError( const psMessage : string ) ;
begin
  MessageDlg( psMessage, mtError, [mbOK], 0 ) ;
end ;
...
//------------------------------------------------------------------------------
function TtiListViewPlus.ValidateFilters( var psMessage : string ; pFilters : TList ) : boolean ;
var
  i : integer ;
begin
  psMessage := '' ;
...
// Apply the group of filters contained in LVConfig.Filters to pValue
//------------------------------------------------------------------------------
function TtiListViewPlus.DoesDataPassFilter(pValue: TPersistent ; pFilters : TList ): boolean;
  // Scan through the filters and add an ExpEval object to the list
  // for each filter.
  //----------------------------------------------------------------------------
  procedure FiltersToExpEval( pFilters, pResult : TList ; pValue : TPersistent ) ;
  var
    i : integer ;
    lExpEval : TtiExpEval ;
  begin
    ...
    for i := 0 to pFilters.Count - 1 do begin
      ...
      lExpEval.Bool := ( TObject( pFilters[i]) as TtiLVFilter ).DoesDataPassFilter( pValue ) ;
...
begin
  ...
  try
    ...
    // bool, conj
    FiltersToExpEval( pFilters, lListExps, pValue ) ;
...
//------------------------------------------------------------------------------
procedure TtiListViewPlus.DoOnFilterData(pData: TPersistent; var pbInclude: boolean);
begin
  pbInclude := DoesDataPassFilter( pData, FLVConfig.Filters ) ;
end;
...
// *
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
procedure TtiEditConditionAbs.AddFieldEdit(pData: TObject);
begin
  // Do nothing, implement in the concrete
end;
...
// -----------------------------------------------------------------------------
procedure TtiEditFilter.AddFieldEdit( pData : TObject ) ;
var
...
// -----------------------------------------------------------------------------
procedure TtiEditFilter.AssignFilters(pList: TList);
var
  i           : integer ;
  lFilterEdit : TtiLVFilterEdit ;
  lFilter     : TtiLVFilter ;
...
// Does this pValue ( a TPersistent descendant ) pass the filter ?
//------------------------------------------------------------------------------
function TtiLVFilter.DoesDataPassFilter(pValue: TPersistent): boolean;
  // The property being filtered is a string
  //----------------------------------------------------------------------------
  function TestStringFilter( pValue : TPersistent ) : boolean ;
  var
    lsProp : string ;
    lsVal  : string ;
  begin
    try
      lsProp := upperCase( GetPropValue( pValue, Field )) ;
    except
      on e:exception do
  ...
  //----------------------------------------------------------------------------
  function TestIntFilter( pValue : TPersistent ) : boolean ;
  var
    lsProp : string ;
    liProp : integer ;
    liVal  : integer ;
  ...
  //----------------------------------------------------------------------------
  function TestFloatFilter( pValue : TPersistent ) : boolean ;
  var
    lsProp : string ;
    lrProp : real ;
    lrVal  : real ;
...
begin
  ...
  lPropType := GetSimplePropType( pValue, Field ) ;
  case lPropType of
  tiTKInteger : result := TestIntFilter(    pValue ) ;
  tiTKFloat   : result := TestFloatFilter(  pValue ) ;
  tiTKString  : result := TestStringFilter( pValue ) ;
  else
...
//------------------------------------------------------------------------------
function TtiLVFilter.WildcardMatch(const pSource,pPattern: String): Boolean;
var
...
begin
  StrPCopy( lSource,  pSource);
  StrPCopy( lPattern, pPattern);
  Result := MatchPattern( lSource, lPattern ) ;
end;

//------------------------------------------------------------------------------
function TtiLVFilter.ValidateFilter( pValue : TPersistent ) : string;
begin
  result := '' ;
  // a) Check the <Field> exists in the object being tested
  if not IsPublishedProp( pValue, Field ) then
...
//------------------------------------------------------------------------------
//procedure TtiLVFindDialog.PositionInListView( piIndex : Integer ) ;
//var
...

{
  ...
  if piIndex > LV.VisibleRowCount then
    LV.Scroll( 0, liItemHeight * piIndex ) ;

  LV.Selected := LV.Items[piIndex] ;
...
//------------------------------------------------------------------------------
procedure TtiListViewPlus.ListToHTMLStream( pStream : TStream );
begin
  WriteTextToStream( pStream, '<html>' ) ;
  WriteTextToStream( pStream, '<head>' ) ;
  WriteTextToStream( pStream, '<title>' + Caption + '</title>' ) ;
  WriteTextToStream( pStream, '</head>' ) ;
  WriteTextToStream( pStream, '<body>' ) ;
  WriteTextToStream( pStream, '<P>' ) ;
  WriteTextToStream( pStream, '<h1><center>' + ListViewCaption + '</center></h1>' ) ;
  WriteTextToStream( pStream, '<p>' ) ;
  WriteTextToStream( pStream, '<table>' ) ;
  WriteTextToStream( pStream, '<table border=1 align="center">' ) ;
  WriteTextToStream( pStream, '<col align="center" span=' +
           IntToStr( ListColumns.Count ) + '>' ) ;
  WriteTextToStream( pStream, '<tr>' ) ;
  ListToStream(      pStream,
                     '<th>', '</th>', '',
                     '<tr>', '<td>', '</td>', '</tr>', '&nbsp;' ) ;
  WriteTextToStream( pStream, '</table>' ) ;
  WriteTextToStream( pStream, '</body>' ) ;
...
//------------------------------------------------------------------------------
procedure TtiEditSort.AddFieldEdit(pData: TObject);
var
...
//------------------------------------------------------------------------------
procedure TtiEditSort.AssignSorts(pList: TList);
var
  i       : integer ;
  lSort   : TtiLVSort ;
  lSortEdit : TtiLVSortEdit ;
...
// *
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
constructor TtiLVClassMapping.CreateExt(const pStrClassID: string;pClassRef: TtiLVClassRef);
begin
  Create ;
...
// -----------------------------------------------------------------------------
procedure TtiLVFactoryAbs.RegisterClass(const pStrClassID: string; pClassRef: TtiLVClassRef);
var i : integer ;
    lClassMapping : TtiLVClassMapping ;
    lStrClassID : string ;
begin
...
// -----------------------------------------------------------------------------
procedure TtiListViewPlus.WriteTextToStream( pStream : TStream ; psText : string ) ;
var
  lpcText : PChar ;
begin
...
{
procedure TtiListViewPlus.ListToCSVStream(pStream: TStream; pList: TList; psSepChar: string );
var
  i, j       : integer ;
  lsValue    : string ;
  lData      : TPersistent ;
...
begin
  ...
  begin
    lColumn := ( ListColsBeingDisplayed.Items[i] as TtiListColumn );
    lColHeading := lColumn.DisplayLabel ;
    lColHeading := StringReplace(lColHeading, psSepChar, '', [rfReplaceAll, rfIgnoreCase]);
    WriteTextToStream( pStream, lColHeading ) ;
    if i < ListColumns.Count - 1 then
      WriteTextToStream( pStream, psSepChar )
    else
      WriteTextToStream( pStream, #13+#10 ) ;
  end ;

  // Write the data
  for i := 0 to pList.Count - 1 do
  begin
    lData := ( TObject( pList.Items[i]) as TPersistent ) ;
    if lData = nil then
    begin
      WriteTextToStream( pStream, #13 ) ;
      Continue ; //==>
    end ;
    for j := 0 to ListColsBeingDisplayed.Count - 1 do
    begin
      lsValue := GetCellText(i, j);
      lsValue := StringReplace(lsValue, psSepChar, '', [rfReplaceAll, rfIgnoreCase]);
      WriteTextToStream( pStream, lsValue ) ;
      if j < ListColumns.Count - 1 then
        WriteTextToStream( pStream, psSepChar )
      else
        WriteTextToStream( pStream, #13+#10 ) ;
    end ;
...
// -----------------------------------------------------------------------------
function TtiLVFilter.GetSimplePropType(pPersistent: TPersistent;
  psPropName: string): TtiLVTypeKind;
var
  lPropType : TTypeKind ;
begin
...
//------------------------------------------------------------------------------
procedure TtiLVMultiSelect.DeSelectByCaption(const psCaption: string; const piPos : integer = -1 );
var
  i : integer ;
  liPos : integer ;
begin
...
//------------------------------------------------------------------------------
procedure TtiLVMultiSelect.SelectByCaption(const psCaption: string; const piPos : integer = -1 );
var
  i : integer ;
  liPos : integer ;
...
begin
  ...
  FlbSelected.Items.Insert( piPos, psCaption ) ;
...
procedure TtiLVConfig.Assign(const pData: TtiLVConfig);
var
  i : integer;
begin
  Assert(pData <> nil, 'pData not assigned' ) ;
  Sorts.Clear;
  for i := 0 to pData.Sorts.Count - 1 do
    Sorts.Add(TtiLVSort(pData.Sorts.Items[i]).Clone);

  Filters.Clear;
  for i := 0 to pData.Filters.Count - 1 do
    Filters.Add(TtiLVFilter(pData.Filters.Items[i]).Clone);

  Finds.Clear;
  for i := 0 to pData.Finds.Count - 1 do
    Finds.Add(TtiLVFilter(pData.Finds.Items[i]).Clone);

  ApplyFilter := pData.ApplyFilter ;
  ApplySort   := pData.ApplySort ;
  if FLV <> nil then
  begin
...
function TtiLVConfig.IsCompatible(const pData: TList): boolean;
var
  i : integer ;
  lData : TPersistent ;
begin
  ...
  if ( pData = nil ) or
     ( pData.Count = 0 ) or
     ( not ( TObject(pData.Items[0]) is TPersistent )) then
    Exit ; //==>

  result := false ;
  lData := TObject( pData.Items[0]) as TPersistent;
...
procedure TtiLVConfig.AssignAvailableColumns(pStrings: TStrings);
var
  i : integer ;
begin
  ...
  for i := 0 to FLV.ListColumns.Count - 1 do
    pStrings.AddObject(FLV.ListColumns.Items[i].DisplayLabel,
                       FLV.ListColumns.Items[i]);
end;

procedure TtiLVConfig.AssignSelectedColumns(pStrings: TStrings);
var
  i : integer ;
begin
  Assert(FLV <> nil, 'LV not assigned' ) ;
  ...
  for i := 0 to FLV.ListColsBeingDisplayed.Count - 1 do
    ...
    begin
      ...
      pStrings.AddObject(TtiListColumn(FLV.ListColsBeingDisplayed.Items[i]).DisplayLabel,
...
procedure TtiListViewPlus.ListToStream(const pStream: TStream;
...
begin
  for i := 0 to ListColsBeingDisplayed.Count - 1 do
    ...
    begin
      WriteTextToStream(pStream, pHeadingSepStart);
      WriteTextToStream( pStream, ( ListColsBeingDisplayed.Items[i] as TtiListColumn ).DisplayLabel ) ;
      WriteTextToStream(pStream, pHeadingSepEnd);
    end ;

  WriteTextToStream( pStream, pHeadingLineSep ) ;

  // Write the data
  for i := 0 to DataInternal.Count - 1 do
  begin
    WriteTextToStream(pStream, pLineSepStart);
    for j := 1 to Columns.Count - 1 do
    begin
      WriteTextToStream(pStream, pDataSepStart);
      ...
      if ls = '' then
        ls := pEmptyChar ;
      WriteTextToStream( pStream, ls + pDataSepEnd ) ;
    end ;
    WriteTextToStream( pStream, pLineSepEnd ) ;
  end ;
...
procedure TtiListViewPlus.INIWriteString(const pSection, pIdent,pValue: string);
var
  lFileName : string ;
  lINI : TINIFile ;
begin
  ...
  lINI := TINIFile.Create(lFileName);
  try
    lINI.WriteString(pSection, pIdent, pValue);
  finally
    lINI.Free;
  end;
end;
