...
type
  ...
  TUserDefinedPickerClick = procedure( var pData : TPersistent ; const pSender : TtiUserDefinedPicker ) of object ;
  ...
  public
    constructor Create( Owner : TComponent ) ; override ;
    property    Value : TPersistent read FValue write SetValue ;
    procedure   LinkToData(pData: TPersistent;const pFieldName, pFieldNameDisplay : string); reintroduce ;
  published
...
begin
{
  ...
  if IsPublishedProp( Data, FieldName ) then
    lValue := (GetObjectProp( Data, FieldName ) as TPersistent )
  else
  begin
...
procedure TtiUserDefinedPicker.LinkToData(pData: TPersistent;
...
procedure TtiUserDefinedPicker.SetValue(const Value: TPersistent);
begin
  FValue := Value;
  DataToWinControl ;
end;
...
begin
  Assert( false, 'You should not be in here' ) ;
end;
