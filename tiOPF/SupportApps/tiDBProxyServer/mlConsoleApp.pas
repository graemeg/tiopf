...
function ExecConsoleApp(const ApplicationName,
                        ...
                        pGetExecutableInfo: Boolean  {Confirm the exe can be run - requires exclusive access to the exe}
                        ): DWORD;
{Parameters
  ApplicationName.
...
begin
  ...
  if not pGetExecutableInfo then
  begin
...
{$ENDIF}
  ...
  if not CreatePipe(ReadHandle, WriteHandle, @SecurityAttributes, 0) then
    // 10/11/2004 PH Added $IFDEF to get compiling in D5
    {$IFDEF DELPHI6ORABOVE}
    RaiseLastOSError;
    {$ELSE}
      raise exception.create('Unknown OS error');
    {$ENDIF}
