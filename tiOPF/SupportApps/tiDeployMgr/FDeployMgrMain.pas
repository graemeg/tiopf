...
uses
  ...
  ToolWin  ,tiDeployMgr_BOM, tiButtons, Menus, tiTreeView
  ;
...
begin
  TV.Data := nil ;
  FData.Clear ;
  gTIPerMgr.VisMgr.Execute( cgsDeployMgrReadPK, FData ) ;
...
begin
  ...
  gTIPerMgr.VisMgr.Execute(     cgsDeployMgrSave, FData ) ;
...
procedure TFormMain.TVSelectNode(ptiTreeView: TtiTreeView;
  ...
  pParentData: TObject);
begin
  gTIPerMgr.VisMgr.Execute( cgsDeployMgrReadAppDetails, TPerObjAbs( pData )) ;
...
begin
  ...
  begin
    gTIPerMgr.VisMgr.Execute( cgsDeployMgrReadAppDetails, FData ) ;
    ReadDeployFromDirty( FData ) ;
    Save ;
    Close ;
  end ;
