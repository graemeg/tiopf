...
type
  ...
  TVisReadDeployDirtyAbs = class( TVisitorAbs )
  protected
  ...
  TVisReadDeployFromReadIcon = class( TVisitorAbs )
  protected
    function  AcceptVisitor : boolean ; override ;
  public
    procedure Execute( const pVisited : TVisitedAbs ) ; override ;
  ...
  TVisWriteDeployToFile = class( TVisitorAbs )
  protected
    function  AcceptVisitor : boolean ; override ;
  public
    procedure Execute( const pVisited : TVisitedAbs ) ; override ;
...
procedure ReadDeployToDirty(          pData : TPerObjAbs ) ;
procedure ReadDeployFromDirty(        pData : TPerObjAbs ) ;
procedure ReadDeployFromFileDetails(  pData : TPerObjAbs ) ;
procedure WriteDeployToFile(          pData : TPerObjAbs ) ;
procedure LaunchFiles(                pData : TPerObjAbs ) ;
procedure ReadDeployFromIcon(         pData : TPerObjAbs ) ;
...
begin
  ...
  try
    ...
    if not gtiPerMgr.DefaultPerLayer.DBConnectionPools.IsConnected( FsDatabaseName ) then
    begin
      Self.Log( 'Connecting to  ' + FsDatabaseName + '...' ) ;
      gtiPerMgr.DefaultPerLayer.DBConnectionPools.Connect( FsDatabaseName ) ;
      Self.Log( 'Connected.' ) ;
    end else
    ...
    gTIPerMgr.VisMgr.Execute( cgsDeployMgrReadAppDetails, FDeployApp ) ;
...
begin
  ...
  begin
    ...
    begin
      ...
      gTIPerMgr.VisMgr.Execute( cgsDeployMgrReadBin, lDeployFile, FsDatabaseName ) ;
...
procedure TVisReadDeployFromDirty.Execute(const pVisited: TVisitedAbs);
var
  lData : TtiDeployFile ;
begin
...
procedure TVisReadDeployToDirty.Execute(const pVisited: TVisitedAbs);
var
  lData : TtiDeployFile ;
begin
  inherited Execute( pVisited ) ;
...
procedure TVisReadDeployFromFileDetails.Execute(const pVisited: TVisitedAbs);
var
  lDT : TDateTime ;
  liSize : integer ;
  lData : TtiDeployFile ;
...
procedure TVisWriteDeployToFile.Execute(const pVisited: TVisitedAbs);
var
  lDeployFile : TtiDeployFile ;
begin
  inherited Execute( pVisited ) ;
  ...
  LogFmt( 'File size: %d File name: %s',
...
begin
  ...
  lData := TtiDeployFile( Visited ) ;
  LogFmt( 'About to launch %s', [lData.DeployToPathAndName] ) ;
                     
  //lsParams := TtiDeployApp( TtiDeployFile( Visited ).Owner ).CommandLineParams ;

  // ToDo: Must do better than this: lsParams := '-d ' + gTIPerMgr.DefaultDBConnectionName
  //lsParams := '-lv -d ' + gTIPerMgr.DefaultDBConnectionName ;
  lsParams := '-d ' + gTIPerMgr.DefaultDBConnectionName ;
...
procedure TVisReadDeployFromReadIcon.Execute(const pVisited: TVisitedAbs);
var
  lDummy : word ;
  lIcon : TIcon ;
  lFileName : string ;
