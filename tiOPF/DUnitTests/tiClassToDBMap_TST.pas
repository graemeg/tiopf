...
type
  ...
  TCTDBMTestObj1  = class( TPerObjAbs ) ;
  TCTDBMTestList1 = class( TPerObjList ) ;
  TCTDBMTestObj2  = class( TPerObjAbs ) ;
  TCTDBMTestList2 = class( TPerObjList ) ;
  TCTDBMTestObj3  = class( TPerObjAbs ) ;
  TCTDBMTestList3 = class( TPerObjList ) ;


  TTestTIClassToDBMapFramework = class( TTestCase )
  private
    procedure AddAttrColMaps(pAttrColMaps: TtiAttrColMaps; pAttrName, pColName: string);
  public
  published
  ...
  TTestTIClassToDBMapOperation = class( TtiPerTestCase )
  private
    FLongString : string ;
    ...
    procedure InsertTIOPFTestDataInheritedGroup(pOID: integer);
  protected
    procedure Setup ; override ;
    procedure TearDown ; override ;
    procedure DoReadWriteString(const pLen : integer);
    procedure DoReadWriteInteger(pValue: integer);
{$IFDEF TESTINT64}
...
begin
  ...
  if gPerFrameworkSetupFactory.TestNonPersistentClasses then
    RegisterTest( TTestTIClassToDBMapFramework.Suite ) ;

  tiDUnitDependencies.RegisterDBTests(
    'Class to DB Map',
    TTestTIClassToDBMapOperation );
...
begin
  ...
  lData1  := CreateTIOPFTestData ;
  lError := gTIPerMgr.Save( lData1, DatabaseName, PerLayerName ) ;
  if lError <> '' then
  ...
  // ToDo: The top object will not have been marked as deleted here
  lOld := tiPerObjAbsAsString( lData1 ) ;
  Check( lOld = '', 'Delete failed' ) ;

  lData2 := TtiOPFTestData.Create ;

  lError := gTIPerMgr.Read( lData2, DatabaseName, PerLayerName  ) ;
  if lError <> '' then
    raise exception.create( lError ) ;

  lNew := tiPerObjAbsAsString( lData2 ) ;
...
procedure TTestTIClassToDBMapOperation.UpdateTIOPFTestData(pData: TtiOPFTestData);
var
  lGroup : TtiOPFTestGroup ;
  lItem  : TtiOPFTestItem  ;
  i, j   : integer ;
...
begin
  ...
  try
    ...
    gTIPerMgr.InsertRow( 'Test_Group', lQP, DatabaseName, PerLayerName ) ;
  finally
    lQP.Free ;
  end ;
...
begin
  ...
  try
    lClassMap.PerObjAbsClass := TPerObjAbs ;
    // This should work...
...
begin
  ...
  try
    lClassMap := lClassMaps.AddClassMap( TPerObjAbs ) ;
    CheckNotNull( lClassMap ) ;
    CheckEquals( TPerObjAbs, lClassMap.PerObjAbsClass, 'Failed on lClassMap.PerObjAbsClass' ) ;
    CheckEquals( 1, lClassMaps.Count, 'Failed on lClassMaps.Count' ) ;
    CheckEquals( TPerObjAbs, lClassMaps.Items[0].PerObjAbsClass, 'Failed on lClassMaps.Items[i].PerObjAbsClass' ) ;
  finally
    lClassMaps.Free ;
  end ;
end;
...
begin
  lClassMaps := TtiClassMaps.Create;
  try
    lClassMap := lClassMaps.FindCreate( TPerObjAbs ) ;
    CheckNotNull( lClassMap ) ;
    CheckEquals( TPerObjAbs, lClassMap.PerObjAbsClass, 'Failed on lClassMap.PerObjAbsClass' ) ;
    CheckEquals( 1, lClassMaps.Count, 'Failed on lClassMaps.Count' ) ;
    CheckEquals( TPerObjAbs, lClassMaps.Items[0].PerObjAbsClass, 'Failed on lClassMaps.Items[i].PerObjAbsClass' ) ;
...
begin
  ...
  try
    lClassMap := lClassMaps.AddClassMap( TPerObjAbs ) ;
    CheckNotNull( lClassMap ) ;
    Check( lClassMaps.IsClassReg( TPerObjAbs ), 'Failed on lClassMaps.IsClassReg( TPerObjAbs )');
    Check( not lClassMaps.IsClassReg( TPerObjList ), 'Failed on not lClassMaps.IsClassReg( TPerObjList )');
  finally
    lClassMaps.Free ;
  end ;
end;
...
procedure TTestTIClassToDBMapFramework.AddAttrColMaps( pAttrColMaps : TtiAttrColMaps ; pAttrName : string ; pColName : string ) ;
var
  lAttrMap : TtiAttrMap ;
  lDBColMap  : TtiDBColMap ;
begin
 lAttrMap := TtiAttrMap.Create ;
 lAttrMap.AttrName := pAttrName ;
 lDBColMap  := TtiDBColMap.Create ;
 lDBColMap.ColName := pColName ;
 pAttrColMaps.AddMapping( lAttrMap, lDBColMap ) ;
end ;
...
begin
  ...
  try
    AddAttrColMaps( lAttrColMaps, 'attr1', 'col1' ) ;
...
begin
  ...
  try
    ...
    lClassMap1.PerObjAbsClass := TPerObjAbs ;
    ...
    lClassMap2.PerObjAbsClass := TPerObjList ;
    ...
    try
      ...
      lAttrColMaps.FindAllMappingsByMapToClass( TPerObjAbs, lFoundList ) ;
      ...
      lAttrColMaps.FindAllMappingsByMapToClass( TPerObjList, lFoundList ) ;
      CheckEquals( 3, lFoundList.Count, 'Failed on lFondList.Count' ) ;
...
begin
  ...
  try
    ...
    lFound := lAttrColMaps.FindByClassAttrMap( TPerObjAbs, 'attr1' ) ;
    CheckNotNull( lFound ) ;
    Check( lFound.AttrMap = lAttrMap11, 'Failed on TPerObjAbs, "attr1"' ) ;
    lFound := lAttrColMaps.FindByClassAttrMap( TPerObjAbs, 'attr2' ) ;
    CheckNotNull( lFound ) ;
    Check( lFound.AttrMap = lAttrMap12, 'Failed on TPerObjAbs, "attr2"' ) ;

    lFound := lAttrColMaps.FindByClassAttrMap( TPerObjList, 'attr1' ) ;
    CheckNotNull( lFound ) ;
    Check( lFound.AttrMap = lAttrMap21, 'Failed on TPerObjList, "attr1"' ) ;

    lFound := lAttrColMaps.FindByClassAttrMap( TPerObjAbs, 'attr4' ) ;
    CheckNull( lFound, 'Failed on TPerObjAbs, "attr4"' ) ;
...
begin
  ...
  try
    lClassDBCollection := lClassDBCollections.AddClassCollectionMapping( TPerObjList, TPerObjAbs ) ;
    CheckEquals( 1, lClassDBCollections.Count, 'Failed on count = 1' ) ;
    CheckNotNull( lClassDBCollection, 'Failed on NotNull' ) ;
    Check( lClassDBCollection.CollectionClass = TPerObjList, 'Failed on lClassDBCollection.CollectionClass = TPerObjList' ) ;
    Check( lClassDBCollection.PerObjAbsClass  = TPerObjAbs, 'Failed on lClassDBCollection.PerObjAbsClass  = TPerObjAbs' ) ;
...
begin
  ...
  try
    ...
    lCDBCs.FindByCollection( TPerObjList, lList ) ;
    CheckEquals( 0, lList.Count, 'Failed not to find collection for TPerObjList' ) ;
...
begin
  ...
  try
    ...
    lCDBC := lCDBCs.FindByCollectionOf( TPerObjAbs ) ;
    CheckNull( lCDBC, 'Failed on lCDBCs.FindByCollectionOf( TPerObjAbs )' ) ;
...
begin
  ...
  try
    ...
    Check( not lCDBCs.IsInCollection( TPerObjAbs ), 'Failed on not TPerObjAbs' ) ;
...
begin
  ...
  try
    lCDBMM.RegisterMapping( 'test', TPerObjList, 'test', 'OID', 'OID', [pktDB]);
    lClassDBCollection := lCDBMM.Collections.AddClassCollectionMapping( TPerObjList, TPerObjAbs{, ['Owner_OID']} ) ;
...
begin
  ...
  try
    ...
    Check( lCDBMM.ClassMaps.Items[1].PerObjAbsClass = TCTDBMTestObj1, 'Failed on TPerObjList' ) ;
    ...
    try
      lCDBMM.RegisterMapping( 'db_3', TPerObjList, 'table_1', 'prop_1', 'col_1', [pktDB]);
...
begin
  ...
  try
    ...
    try
      lFoundList.OwnsObjects := false ;
      lAttrColMaps.FindAllPKMappingsByMapToClass( TPerObjAbs, lFoundList ) ;
      CheckEquals( 1, lFoundList.Count, 'Failed on lFondList.Count' ) ;
      Check( lFoundList.Items[0].AttrMap = lAttrMap11, 'Failed on lFoundList.Items[0].AttrMap = lAttrMap11' ) ;

      lAttrColMaps.FindAllPKMappingsByMapToClass( TPerObjList, lFoundList ) ;
...
begin
  ...
  try
    ...
    Check(not lCDBCs.IsCollection(TPerObjList), 'Failed on 4');
...
procedure TTestTIClassToDBMapOperation.InsertTIOPFTestData;
  ...
  begin
    ...
    try
      ...
      gTIPerMgr.InsertRow( 'Test_Group', lQueryParams, DatabaseName, PerLayerName  ) ;
    finally
      lQueryParams.Free ;
    end ;
  end ;

  procedure _InsertItem( pOID, pI, pJ : integer ) ;
  var
    lQueryParams : TtiQueryParams ;
  begin
    lQueryParams := TtiQueryParams.Create ;
    try
      lQueryParams.SetValueAsString('OID',              IntToStr(pOID)) ;
      ...
      gTIPerMgr.InsertRow( 'Test_Item', lQueryParams, DatabaseName, PerLayerName  ) ;
    finally
      lQueryParams.Free ;
    end ;
  end ;
...
procedure TTestTIClassToDBMapOperation.InsertTIOPFTestDataInheritedGroup( pOID : integer );
var
  lQueryParams : TtiQueryParams ;
begin
  lQueryParams := TtiQueryParams.Create ;
  try
    lQueryParams.SetValueAsString('OID', IntToStr(pOID)) ;
    gTIPerMgr.InsertRow( cTableNameTIOPFTestParentGroup, lQueryParams, DatabaseName, PerLayerName  ) ;
  finally
    lQueryParams.Free ;
  end ;
end ;

procedure TTestTIClassToDBMapOperation.InsertTIOPFTestDataInherited(const pParentTableName, pTableName : string ; pI: Integer ; pOwnerOID : integer );
  procedure _InsertTestParent( const pTableName : string ; pI : integer ; pOwnerOID : integer ) ;
  var
    lQueryParams : TtiQueryParams ;
  begin
    lQueryParams := TtiQueryParams.Create ;
    try
      ...
      gTIPerMgr.InsertRow( pTableName, lQueryParams, DatabaseName, PerLayerName  ) ;
    finally
      lQueryParams.Free ;
    end ;
  end ;

  procedure _InsertTestChild( const pTableName : TTableName ; pI : integer ) ;
  var
    lQueryParams : TtiQueryParams ;
  begin
    lQueryParams := TtiQueryParams.Create ;
...
begin
  ...
  _InsertTestChild( pTableName, pI ) ;
end;
...
begin
  ...
  try
    try
      lClassMaps.RegisterInheritance( TPerObjAbs, TPerObjList ) ;
    ...
    lClassMapParent := lClassMaps.AddClassMap( TPerObjAbs ) ;
    CheckNotNull( lClassMapParent, 'Failed creating classmap for TPerObjAbs' ) ;
    try
      lClassMaps.RegisterInheritance( TPerObjAbs, TPerObjList ) ;
    ...
    lClassMapChild := lClassMaps.AddClassMap( TPerObjList ) ;
    CheckNotNull( lClassMapChild, 'Failed creating classmap for TPerObjList' ) ;

    try
      lClassMaps.RegisterInheritance( TPerObjList, TPerObjAbs ) ;
...
begin
  ...
  try
    ...
    lClassMapChild := lClassMaps.AddClassMap( TPerObjList ) ;
    lClassMaps.RegisterInheritance( TPerObjAbs, TPerObjList ) ;
    lClassMap := lClassMaps.FindParent( TPerObjList ) ;
    Check( lClassMap = lClassMapParent, 'Parent classmap not correctly found' ) ;
    Check( lClassMap.PerObjAbsClass = TPerObjAbs, 'Parent classmap.PerObjClass not of correct type' ) ;
...
begin
  lClassMaps := TtiClassMaps.Create;
  try
    lClassMaps.AddClassMap( TPerObjList ) ;
    lClassMaps.AddClassMap( TPerObjAbs ) ;
    Check((Not lClassMaps.HasParent( TPerObjList )), 'Failed on 1' );
    Check((Not lClassMaps.HasParent( TPerObjAbs )), 'Failed on 2' );
    lClassMaps.RegisterInheritance( TPerObjAbs, TPerObjList ) ;
    Check((Not lClassMaps.HasParent( TPerObjAbs )), 'Failed on 3' );
    Check( lClassMaps.HasParent( TPerObjList ), 'Failed on 4' );
  finally
    lClassMaps.Free;
  end;
end;
...
begin
  ...
  try
    ...
    try
      ...
      lChild := lClassMaps.AddClassMap( TPerObjList ) ;
      lClassMaps.FindAllParents( TPerObjList, lParents ) ;
      CheckEquals( 1, lParents.Count, 'Failed on 1' ) ;

      lParent := lClassMaps.AddClassMap( TPerObjAbs ) ;
      lClassMaps.RegisterInheritance( TPerObjAbs, TPerObjList ) ;
      lClassMaps.FindAllParents( TPerObjList, lParents ) ;
...
constructor TTestTIClassToDBMapOperation.Create(MethodName: string);
begin
  inherited;
...
begin
  Log(ClassName + '.TestSetupAndTearDown');
end;
...
procedure TTestTIClassToDBMapOperation.DoReadWriteBoolean(pValue : boolean);
var
...
begin
  ...
  try
    ...
    try
      ...
      lData.Read( DatabaseName, PerLayerName ) ;
      Check( lData.ObjectState = posClean, 'Failed on ObjectState = posClean' ) ;
      CheckEquals( pValue, lData.BoolField ) ;
    finally
      lData.Free ;
    end ;
  finally
...
procedure TTestTIClassToDBMapOperation.DoReadWriteDateTime(pValue : TDateTime);
var
...
begin
  ...
  try
    ...
    try
      ...
      lData.Read( DatabaseName, PerLayerName ) ;
      Check( lData.ObjectState = posClean, 'Failed on ObjectState = posClean' ) ;
      CheckEquals( pValue, lData.DateField, 0.0001 ) ;
    finally
      lData.Free ;
    end ;
  finally
...
begin
  ...
  try
    ...
    try
      ...
      lData.Read( DatabaseName, PerLayerName ) ;
      Check( lData.ObjectState = posClean, 'Failed on ObjectState = posClean' ) ;
      CheckEquals( pValue, lData.FloatField, 0.0001 ) ;
    finally
      lData.Free ;
    end ;
  finally
...
procedure TTestTIClassToDBMapOperation.DoReadWriteInteger(pValue : integer);
var
...
begin
  ...
  try
    ...
    try
      ...
      lData.Read( DatabaseName, PerLayerName ) ;
      Check( lData.ObjectState = posClean, 'Failed on ObjectState = posClean' ) ;
      CheckEquals( pValue, lData.IntField ) ;
    finally
...
{$IFDEF TESTINT64}
procedure TTestTIClassToDBMapOperation.DoReadWriteInt64(pValue: Int64);
var
...
//  CreateTableInt64(DatabaseName, PerLayerName);
  ...
  try
    ...
    gTIPerMgr.DeleteRow( cTIQueryTableNameInt64, nil ) ;
    lData := TtiOPFTestInt64Prop.Create;
    try
      lData.ObjectState := posCreate ;
      lData.OID.AsString := '1' ;
      lData.Int64Field   := pValue ;
      lData.Save( DatabaseName, PerLayerName ) ;
      Check( lData.ObjectState = posClean, 'Failed on ObjectState = posClean' ) ;
    finally
      lData.Free ;
    ...
    try
      ...
      lData.Read( DatabaseName, PerLayerName ) ;
      Check( lData.ObjectState = posClean, 'Failed on ObjectState = posClean' ) ;
      CheckEquals( pValue, lData.Int64Field ) ;
    finally
