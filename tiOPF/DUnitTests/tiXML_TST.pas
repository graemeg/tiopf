    ...
    // These will give an indication of how much faster you are
{
  ...
  TTestTIXMLParser = class( TTestCase )
  private
    FNode : string ;
...
begin
  if gPerFrameworkSetupFactory.TestNonPersistentClasses then
    RegisterTest( TTestTIXML.Suite );
  if gPerFrameworkSetupFactory.TestNonPersistentClasses then
    RegisterTest( TTestTIXMLParser.Suite );
end;
...
constructor TTestTIXML.Create(MethodName: string);
begin
...
constructor TTestTIXMLParser.Create(MethodName: string);
begin
  inherited;
