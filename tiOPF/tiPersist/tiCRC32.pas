...
function GetMemoryStreamCrc32(Stream: TStream): integer;
function AsmGetMemoryStreamCrc32(Stream: TMemoryStream): integer;
function StringToCRC32( const pString : string ) : integer ;
...
type
 ...
 TCrc32Table = array[0..255] of integer;
...
begin
 Result:=$FFFFFFFF;
 while Stream.Read(Value,1)=1 do Result:=((Result shr 8) and $00FFFFFF) xor Crc32Table[(Result xor Value) and $000000FF];
 Result:=not Result;
end;
...
begin
  lStringStream := TStringStream.Create( pString ) ;
  try
    lStringStream.Position := 0 ;
    result := GetMemoryStreamCrc32( lStringStream ) ;
  finally
    lStringStream.Free ;
  end ;
end ;
