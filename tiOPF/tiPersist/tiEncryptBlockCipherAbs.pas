{ * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  ...
  Classes:
    TtiEncryptBlockCipherAbs - Base class for DES and Blow Fish encryption
  ...
  Acknowledgements:
     ...
     * TBlockCipher inherited from TObject, TtiEncryptBlockCipherAbs inherits from TtiEncryptAbs
     * T64BitBlockCipher merged with TBlockCipher to create TtiEncryptBlockCipherAbs
...
type
  ...
  TtiEncryptBlockCipherAbs = class(TtiEncryptAbs)
  private
  protected
  public
...
{ TtiEncryptBlockCipherAbs }

class function TtiEncryptBlockCipherAbs.CiphertextLength(const pPlaintextLength: Integer): Longint;
begin
  Result := Succ(pPlaintextLength div BlockSize) * BlockSize;
end;

class function TtiEncryptBlockCipherAbs.BlockSize: Integer;
begin
  Result := SizeOf(Int64);
end;

constructor TtiEncryptBlockCipherAbs.Create;
var
  lKey : Int64;
begin
  inherited;
...
procedure TtiEncryptBlockCipherAbs.DecryptStream(const pSrc, pDest: TStream);
var
  Count: Longint;
...
procedure TtiEncryptBlockCipherAbs.EncryptStream(const pSrc, pDest: TStream);
var
  Count: Longint;
...
function TtiEncryptBlockCipherAbs.EncryptString(const psData: string): string;
var
  lPS, lPD: PInt64;
  lSource: Int64;
  I: Integer;
...
function TtiEncryptBlockCipherAbs.DecryptString(const psData: string): string;
var
  lDest: Int64;
  lPS, lPD: PInt64;
  I: Integer;
