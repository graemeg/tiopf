...
type
  ...
  TNextOIDHexData = class( TPerObjAbs )
  private
...
procedure TOIDHex.AssignFromTIQuery(const pFieldName: string; const pQuery: TtiObjAbs);
var
  lQuery : TtiQuery ;
begin
...
procedure TOIDHex.AssignToTIQueryParam(const pFieldName: string;const pParams: TtiObjAbs);
var
...
function TOIDHex.EqualsQueryField(const pFieldName: string;
  const pQuery: TtiObjAbs): boolean;
var
  lQuery : TtiQuery ;
begin
  Assert( pQuery is TtiQuery, 'pQuery not a TtiQuery' ) ;
...
begin
  if FDirty then
  begin
    gTIPerMgr.VisMgr.Execute( cgsNextOIDHexReadHigh, FNextOIDHexData ) ;
    FDirty := false ;
...
procedure TVisDBNextOIDHexAmblerRead.Execute(const pData: TVisitedAbs);
begin
...
procedure TVisDBNextOIDHexAmblerUpdate.Execute(const pData: TVisitedAbs);
var
  lParams : TtiQueryParams ;
  lHex : ShortString;
begin
...
initialization
  ...
  gTIPerMgr.OIDFactory.RegisterMapping( cOIDClassNameHex, TOIDHex, TNextOIDGeneratorHex )  ;

  if gTIPerMgr.DefaultOIDClassName = '' then
    gTIPerMgr.DefaultOIDClassName := cOIDClassNameHex ;

  gTIPerMgr.VisMgr.RegisterVisitor( cgsNextOIDHexReadHigh, TVisDBNextOIDHexAmblerRead ) ;
  gTIPerMgr.VisMgr.RegisterVisitor( cgsNextOIDHexReadHigh, TVisDBNextOIDHexAmblerUpdate ) ;
