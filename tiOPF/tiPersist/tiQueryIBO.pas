{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  ...
  Revision history:
    ...
    August 2002,   Ian Krigsman
                    Implementation of ...AsInteger changed to AsInt64
                    ( which is supported by IBSQL - but applies to SQL dialect 3 only)
                    Tip: Fields in IB6 (dialect 3) declared above Numeric( 9,0 )
                         will be stored as Int64.
                    Cater for booleans defined as CHAR( 1 ) using conditional
                    compilation:-
                      $IFDEF BOOLEAN_CHAR_1 ==> booleans are CHAR( 1 ) 'T' or 'F'
                      otherwise ==> booleans are VARCHAR( 5 ) containing 'TRUE', etc.
                    NOTE: It is alleged that GetFieldAsVariant() will break with
                          booleans as CHAR( 1 ).  (This is the underlying reason
                          for the change in the first place)
    September 2002, Peter Hinrichsen, Modified SetParamAsVariant to work with DateTime(s)
    October 2002,   Lukasz Zeligowski, modified AssignParams to work propertly
                    with IB where SQLDialect is 1 (IB Versions 5.5)

    June 2003, Carlos Augusto, Derived IBObjects layer from IBX layer
  ...
  ToDo:
    Abstract to include a TtiQueryDataSetAbs class which can implement
    the TDataSet functionality that is shared between this unit and
    TtiQueryBDEAbs.
...
begin
  try
    ...
    begin
      ...
      tiFmtException( e, lErrorMessage, ClassName, 'ExecSQL' ) ;
    end;
  end;
end ;
...
function TtiQueryIBO.GetFieldAsFloat ( const psName: string ) : Real ;
begin
...
begin
  {  try
      result := FIBSQL.FieldByName(psName).Value;
    except
      on e: exception do
        tiFmtException(e,
          ClassName,
...
function TtiQueryIBO.GetParamAsFloat ( const psName: string ) : Real ;
begin
...
begin
  ...
  case PSQLVAR.sqltype of
    ...
    tiFmtException ( 'Invalid Interbase DataType <' +
      IntToStr ( PSQLVAR.sqltype ) + '>', ClassName, 'FieldKind' ) ;
  end ;
...
{$ENDIF} // BOOLEAN_CHAR_1
  ...
  //    end;
    ...
    on e: exception do
      tiFmtException ( e, 'ParameterName: ' + psName, ClassName, 'SetParamAsVariant' ) ;
  end ;
end ;
...
procedure TtiDatabaseIBO.ReadMetaDataFields ( pData: TPersistent ) ;
var
  lTableName: string ;
  lQuery: TtiQuery ;
...
begin
  ...
  lQuery := gtiPerMgr.RegPerLayers.CreateTIQuery ;
  try
    StartTransaction ;
    try
...
procedure TtiDatabaseIBO.ReadMetaDataTables ( pData: TPersistent ) ;
var
  lQuery: TtiQuery ;
...
{ sorry, but i need it, and not thinked a better way to do it - Carlos }

{function TtiDatabaseIBO.IBFieldTypeToTIQueryFieldKind( const pIBFieldType: string): TtiQueryFieldKind;
begin

  if ( pIBFieldType = 'TEXT' ) or
     ( pIBFieldType = 'VARYING' ) then
    result := Tqu
  else if
     ( pIBFieldType = 'SHORT') or
     ( pIBFieldType = 'LONG') or
     ( pIBFieldType = 'QUAD') or
     ( pIBFieldType = 'FLOAT') or
     ( pIBFieldType = 'DOUBLE') then
  else if
     ( pIBFieldType = 'TIMESTAMP') or
     ( pIBFieldType = 'DATE') or
     ( pIBFieldType = 'TIME') then
  else
    tiFmtException( 'Error converting Interbase field type to tiQueryFieldKind <' +
                    pIBFieldType + '>' ) ;
  end ;
...
finalization
  gtiPerMgr.RegPerLayers.UnRegisterPersistenceLayer ( cTIPersistIBO ) ;
