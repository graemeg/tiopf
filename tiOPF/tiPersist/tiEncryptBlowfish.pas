...
type
  ...
  TtiBlowfishSBox   = array [Byte] of DWORD;
  TtiBlowfishPArray = array [0..17] of DWORD;

  TEncryptBlowfish = class(TtiEncryptBlockCipherAbs)
  private
...
procedure TEncryptBlowfish.DecryptBlock(var pCiphertext);
var
  L, R: DWORD;
begin
  L := TDoubleDWORD(pCiphertext).L;
  R := TDoubleDWORD(pCiphertext).R;
  L := L xor FPArray[17];
  ...
  TDoubleDWORD(pCiphertext).L := R xor FPArray[0];
  TDoubleDWORD(pCiphertext).R := L xor FPArray[1];
end;

function TEncryptBlowfish.DecryptedBlock(const pCiphertext: Int64): Int64;
var
  L, R: DWORD;
begin
  L := TDoubleDWORD(pCiphertext).L;
...
procedure TEncryptBlowfish.EncryptBlock(var pPlaintext);
var
  L, R: DWORD;
begin
  L := TDoubleDWORD(pPlaintext).L;
  R := TDoubleDWORD(pPlaintext).R;
  L := L xor FPArray[0];
  with TFourByte(L) do begin
    R := R xor (FSBox1[B4] + FSBox2[B3] xor FSBox3[B2] + FSBox4[B1]);
  end;
  ...
  TDoubleDWORD(pPlaintext).L := R xor FPArray[17];
  TDoubleDWORD(pPlaintext).R := L xor FPArray[16];
end;

function TEncryptBlowfish.EncryptedBlock(const pPlaintext: Int64): Int64;
var
  L, R: DWORD;
begin
  L := TDoubleDWORD(pPlaintext).L;
