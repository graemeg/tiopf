...
type
  ...
  // ---------------------------------------------------------------------------
  TLogToError = class( TLogToCacheAbs )
  private
    FForm              : TLogErrorForm ;
...
begin
  ...
  for i := 0 to ListWorking.Count - 1 do begin
    ...
    FForm.MemoLog.Lines.Add( '' ) ;
    lsMessage := tiLog._StrTran( lLogEvent.LogMessage, #10, '' ) ;
    lsMessage := tiLog._StrTran( lsMessage, #13, #13+#10 ) ;
...
procedure TLogToError.Log(const psDateTime, psThreadID, psMessage: string; pSeverity: TLogSeverity);
begin
  if Terminated then
    Exit ; //==>
