...
function  tiDataSetToString( pDataSet : TtiDataSet ) : string ;
procedure tiDataSetToTextFile( pDataSet : TtiDataSet ; pFileName : TFileName ) ;
procedure tiDataSetToListView( pDataSet : TtiDataSet ; pLV : TListView ) ;
procedure tiDataSetToListItem( pDataSet : TtiDataSet ; pItem : TListItem ) ;
function  tiDataSetToHTML(     const pDataSet : TtiDataSet ) : string ;
function  tiDataSetToHTMLV(    const pDataSet : TtiDataSet ) : string ;
procedure ShowTIDataSet(       const pDataSet : TtiDataSet ) ; // For debugging
...
begin
  ...
  {$IFDEF MSWINDOWS}
  pLV.Items.Count := 0 ;      { this is read-only under Linux }
  {$ENDIF MSWINDOWS}
  pLV.Columns.Clear ;
  ...
  begin
    ...
    {$IFDEF KYLIX}
      lTextSize := tiGetTextSize( pLV.Font, pDataSet.Fields.Items[ i ].Name );
      lCol.Width    := Trunc( lTextSize * 1.2 ) ;
    {$ELSE}
...
begin
  ...
  {$ELSE}
    lTextSize.cx := ( pItem.TextRect.Right - pItem.TextRect.Left );
    lTextSize.cy := ( pItem.TextRect.Top - pItem.TextRect.Bottom );
  {$ENDIF MSWINDOWS}
