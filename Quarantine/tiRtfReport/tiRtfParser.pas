...
begin
  if Exponent = 0.0
  then Result := 1.0 { n**0 = 1 }
  else if(Base = 0.0)and(Exponent > 0.0)
  then Result := 0.0 { 0**n = 0, n > 0 }
  else if(Frac(Exponent) = 0.0)and(Abs(Exponent) <= MaxInt)
  then Result := ipwr(Base, integer(Trunc(Exponent)))
...
begin
  Result := Find(AName);
  if Assigned(Result)
  then raise TRtfException.CreateFmt('Variable %s already exists',[AName]);
...
begin
  ...
  end else if Dataset is TPerObjList
  then Result := TableIndex = 0
  else if Dataset is TPerObjAbs
  then Result := false
  else raise TRtfException.Create(rsNotImplemented);
end;
...
begin
  ...
  then Result :=(TableIndex >= RecordCount)or(TableIndex < 0)
  else if Dataset is TPerObjAbs
  then Result := false
  else Result := false;
end;
...
begin
  Result := Find(AName);
  if Assigned(Result)
  then raise TRtfException.CreateFmt('Dataset already exists',[AName]);
...
begin
  ...
  while AIndex < Length(AName)do begin
    ...
    if Result.Dataset is TPerObjList then begin
      //Get the right record from the array
      if Result.TableIndex <(Result.Dataset as TPerObjList).Count
      then AObject :=(Result.Dataset as TPerObjList)[Result.TableIndex]
      else exit; //Trying beyond eof (or empty table) big problem; nah?
    end else if Result.Dataset is TPerObjAbs
...
begin
  ...
  end else if Dataset is TPerObjList then begin
    TableIndex := 0;
  end;
end;
...
begin
  ...
  else if Dataset is TPerObjList
  then Result :=(Dataset as TPerObjList).Count
  ...
  while FSourcePtr^ <> #0 do begin
    ...
    case P^ of
      ...
      '0'..'9': begin //Ik ga ervan uit dat alles in het amerikaans genoteerd word
        ...
        SetString(AToken, TokenStart, p - TokenStart);
        if AFloatResult
        then AValue := StrToFloat(AToken, Parser.AFormatSettings)
...
begin
  ...
  else raise TRtfException.Create('Graphic fields worden alleen ondersteund via de tdataset!');
end;
...
var AObject: TObject;
  ...
  AField: TField;
begin
  if AFieldName = '' //Can't resolve that
  then raise TRtfException.Create('invalid fieldname');
  ...
  if ADataset.Dataset is TDataset then begin
    ...
    case AField.DataType of
      ...
      ftBoolean: begin
        ...
        if Value
        then Token := etLitTrue
        else Token := etLitFalse;
      end;
      ...
      else raise TRtfException.CreateFmt('Unable to convert field "%s" value',[AFieldName]);
    end;
  end else begin
    AObject := ADataset.Dataset;
    if AObject is TPerObjList then begin
      //Get the right record from the array
      if ADataset.TableIndex <(AObject as TPerObjList).Count
      then AObject :=(AObject as TPerObjList)[ADataset.TableIndex]
      else begin //Trying beyond eof (or empty dataset) big problem; nah?
        Token := etNothing;
        Value := varEmpty;
        exit;
      end;
    ...
    if not Assigned(APropInfo)
    then raise TRtfException.CreateFmt('Field "%s" does not exist',[AFieldName]);
    if not Assigned(APropInfo^.GetProc)
    then raise TRtfException.CreateFmt('Cannot access field "%s"',[AFieldName]);
    ATypeData := GetTypeData(APropInfo^.PropType^);

    case APropInfo^.PropType^.Kind of
      tkChar, tkString, tkLString, tkWString: begin
        Token := etLitString;
      ...
      tkEnumeration: begin
        if ATypeData^.BaseType^ = TypeInfo(boolean) then begin
      ...
      tkFloat: begin
        if SameText(APropInfo^.PropType^.Name, 'TDate') or SameText(APropInfo^.PropType^.Name, 'TTime') or SameText(APropInfo^.PropType^.Name, 'TDateTime')
        then Token := etLitDate
        else Token := etLitFloat;
...
begin
  if Count > 0 then begin
    ...
    Walk([etMul, etDiv], EvaluateUnaryBinary);
    //Secondly the unary operators
    Walk([etAdd, etSub], EvaluateUnaryBinary);
    //And finally the simple comparisons
    Walk([etEQ, etNE, etGE, etLE, etGT, etLT], EvaluateComparison);
    //And finally the more "complicated" comparisons
    Walk([etAnd, etOr], EvaluateComparison);
  end;
  ...
  if Count > 0
  then Walk([etAssign], EvaluateAssign);
end;
...
begin
  ...
  for i := 1 to Count - 1 do begin
    AColor := Items[i];
    if(AColor.Red = ARed)and(AColor.Green = AGreen)and(AColor.Blue = ABlue) then begin
      Result := i;
      exit;
    end;
  end;
...
begin
  ...
  while Assigned(AItem)do begin
    case AItem.RtfClass of
      RtfText: begin
        if not AEnter and(Pos(#13#10, AItem.RtfTextBuf) > 0) then begin
...
begin
  for i := 0 to AItems.Count - 1 do begin
    AItem := AItems[i];
    if AItem.RtfClass = RtfExpression
    then ParseExpression(AItem)
    else AddToRtfItems(AItem);
  end;
end;

procedure TtiRtfParser.ParseExpression(AItem: TRtfItem);
var i, ABackColor, AForeColor: integer;
  ARemove: boolean;
  ADataset: TRtfDataset;
  AScanNoEof: boolean;
  AExpression: string;
  ...
  try
    ...
    try
      ...
      if AArguments.Count > 0 then begin
        ...
        if(AArguments[0].Token = etVariable)and(AArguments.Count = 1)
        then ARemove := false; //But show a single variable
      end;
      if Assigned(OnEvalutate) //debug
      then OnEvalutate(AArguments);
      AArguments.Evaluate;
      ...
      case AItem.RtfMajor of
        RtfNormalExpression: begin
          if not ARemove then begin
            ...
            if AResultToken = etLitTrue
            then AAnswer.RtfTextBuf := BoolTrue
            else if AResultToken = etLitFalse
            then AAnswer.RtfTextBuf := BoolFalse
            else if AResultToken = etLitDate then begin
              if Frac(AResultValue) = 0
              then AAnswer.RtfTextBuf := DateToStr(AResultValue, AFormatSettings)
              else if Int(AResultValue) = 0
              then AAnswer.RtfTextBuf := TimeToStr(AResultValue, AFormatSettings)
              else AAnswer.RtfTextBuf := DateTimeToStr(AResultValue, AFormatSettings)
            end
            else AAnswer.RtfTextBuf := VarToStr(AResultValue);
          end;
        end;
        RtfIfExpression: begin
          if AResultToken = etLitTrue
          then Parse(AItem[0])
          else if(AResultToken = etLitFalse)and(AItem.Count > 1)
          then Parse(AItem[1]);
        end;
        RtfScan: begin
...
begin
  ...
  FFunctions.Add(etFunction, 'Year', 1, 1, UdfYear);
  FFunctions.Add(etFunction, 'Month', 1, 1, UdfMonth);
  FFunctions.Add(etFunction, 'Day', 1, 1, UdfDay);
  FFunctions.Add(etFunction, 'SYear', 1, 1, UdfSYear);
  FFunctions.Add(etFunction, 'SMonth', 1, 1, UdfSMonth);
  FFunctions.Add(etFunction, 'SDay', 1, 1, UdfSDay);
  FFunctions.Add(etFunction, 'Dtos', 1, 2, UdfDtos);
  FFunctions.Add(etFunction, 'Stod', 1, 1, UdfStod);
  FFunctions.Add(etFunction, 'DateToStr', 1, 2, UdfDateToStr);
  FFunctions.Add(etFunction, 'TimeToStr', 1, 2, UdfTimeToStr);
  FFunctions.Add(etFunction, 'DateTimeToStr', 1, 2, UdfDateTimeToStr);
  FFunctions.Add(etFunction, 'StrToDate', 1, 1, UdfStrToDate);
  FFunctions.Add(etFunction, 'StrToTime', 1, 1, UdfStrToTime);
  FFunctions.Add(etFunction, 'StrToDateTime', 1, 1, UdfStrToDateTime);
  FFunctions.Add(etFunction, 'ShortMonthName', 1, 1, UdfShortMonthName);
  FFunctions.Add(etFunction, 'ShortDayName', 1, 1, UdfShortDayName);
  FFunctions.Add(etFunction, 'LongMonthName', 1, 1, UdfLongMonthName);
  FFunctions.Add(etFunction, 'LongDayName', 1, 1, UdfLongDayName);

  //String, int, float functions/conversions
  FFunctions.Add(etFunction, 'Int', 1, 1, UdfInt);
  FFunctions.Add(etFunction, 'Chr', 1, 1, UdfChr);
  FFunctions.Add(etFunction, 'Iif', 3, 3, UdfIif);
  FFunctions.Add(etFunction, 'Str', 1, 1, UdfStr);
  FFunctions.Add(etFunction, 'Val', 1, 1, UdfVal);
  FFunctions.Add(etFunction, 'Nul', 2, 2, UdfNul);
  FFunctions.Add(etFunction, 'Now', 0, 0, UdfNow);
  FFunctions.Add(etFunction, 'Frac', 1, 1, UdfFrac);
  FFunctions.Add(etFunction, 'Padr', 2, 2, UdfPadr);
  FFunctions.Add(etFunction, 'Padl', 2, 2, UdfPadl);
  FFunctions.Add(etFunction, 'Date', 0, 0, UdfDate);
  FFunctions.Add(etFunction, 'Time', 0, 0, UdfTime);
  FFunctions.Add(etFunction, 'Mid', 2, 3, UdfSubStr);
  FFunctions.Add(etFunction, 'Copy', 2, 3, UdfSubStr);
  FFunctions.Add(etFunction, 'SubStr', 2, 3, UdfSubStr);
  FFunctions.Add(etFunction, 'Trunc', 1, 1, UdfTrunc);
  FFunctions.Add(etFunction, 'Round', 1, 2, UdfRound);
  FFunctions.Add(etFunction, 'Upper', 1, 1, UdfUpper);
  FFunctions.Add(etFunction, 'Lower', 1, 1, UdfLower);
  FFunctions.Add(etFunction, 'Trim', 1, 1, UdfTrim);
  FFunctions.Add(etFunction, 'Empty', 1, 1, UdfEmpty);
  FFunctions.Add(etFunction, 'TrimLeft', 1, 1, UdfTrimLeft);
  FFunctions.Add(etFunction, 'TrimRight', 1, 1, UdfTrimRight);
  FFunctions.Add(etFunction, 'IntToStr', 1, 1, UdfIntToStr);
  FFunctions.Add(etFunction, 'StrToInt', 1, 1, UdfStrToInt);
  FFunctions.Add(etFunction, 'FloatToStr', 1, 1, UdfFloatToStr);
  FFunctions.Add(etFunction, 'StrToFloat', 1, 1, UdfStrToFloat);
  FFunctions.Add(etFunction, 'Power', 2, 2, UdfIntPower);
  FFunctions.Add(etFunction, 'IntPower', 2, 2, UdfIntPower);
  FFunctions.Add(etFunction, 'FormatFloat', 2, 2, UdfFormatFloat);
  FFunctions.Add(etFunction, 'FBool', 1, 1, UdfFbool);

  //Dataset
  FFunctions.Add(etFunction, 'Bof', 1, 1, UdfBof);
  FFunctions.Add(etFunction, 'Eof', 1, 1, UdfEof);
  FFunctions.Add(etProcedure, 'Next', 1, 1, UdfNext);
  FFunctions.Add(etProcedure, 'Prev', 1, 1, UdfPrior);
  FFunctions.Add(etProcedure, 'Open', 1, 1, UdfOpen);
  FFunctions.Add(etProcedure, 'First', 1, 1, UdfFirst);
  FFunctions.Add(etProcedure, 'Last', 1, 1, UdfLast);
  FFunctions.Add(etFunction, 'IsEmpty', 1, 1, UdfIsEmpty);
  FFunctions.Add(etFunction, 'RecordCount', 1, 1, UdfRecordCount);
  FFunctions.Add(etProcedure, 'Scan', 1, 9999, UdfScan);
  FFunctions.Add(etProcedure, 'EndScan', 0, 0, UdfDummy);
  FFunctions.Add(etProcedure, 'ScanEntry', 0, 0, UdfDummy);
  FFunctions.Add(etProcedure, 'ScanFooter', 0, 0, UdfDummy);
  FFunctions.Add(etProcedure, 'Dataset', 3, 9999, UdfDataset);

  //Misc routines
  FFunctions.Add(etProcedure, 'If', 1, 9999, UdfIf);
  FFunctions.Add(etProcedure, 'Else', 0, 0, UdfDummy);
  FFunctions.Add(etProcedure, 'Endif', 0, 0, UdfDummy);
  FFunctions.Add(etProcedure, 'NoPar', 0, 0, UdfDummy);
  FFunctions.Add(etFunction, 'Picture', 1, 1, UdfPicture);
  FFunctions.Add(etFunction, 'DbPicture', 1, 1, UdfDbPicture);
end;
...
begin
  ...
  then raise TRtfException.Create(rsUnexpectedParameterType);
  AArgument.Value := FloatToStr(AArgument[0].Value, AFormatSettings);
  AArgument.Token := etLitString;
end;
...
begin
  if not AArgument.Check(0,[etLitString])
  then raise TRtfException.Create(rsUnexpectedParameterType);
  AArgument.Value := StrToFloat(AArgument[0].Value, AFormatSettings);
...
begin
  ...
  if AArgument.Count > 2 then begin
    if not AArgument.Check(2,[etLitInt])
    then raise TRtfException.Create(rsUnexpectedParameterType);
...
begin
  if not AArgument.Check(0,[etLitString])
  then raise TRtfException.Create(rsUnexpectedParameterType);
  if(Pos('.', AArgument[0].Value) > 0)or(Pos(',', AArgument[0].Value) > 0) then begin
    AArgument.Token := etLitString;
    AArgument.Value := StrToFloat(AArgument[0].Value, AFormatSettings)
  end else begin
    AArgument.Token := etLitString;
...
var ASign, ADecimals, AInt: integer;
...
begin
  ...
  then raise TRtfException.Create(rsUnexpectedParameterType);
  if AArgument.Count > 1 then begin
    if not AArgument.Check(1,[etLitInt])
    then raise TRtfException.Create(rsUnexpectedParameterType);
  ...
  if AArgument[0].Token = etLitFloat then begin
    ...
    if ADecimals = 0 then begin
      AInt := Trunc(AFloat); //integer part
      AFrac := AFloat - AInt; //fractional part
      if AFrac >= 0.5
      then AArgument.Value := ASign *(AInt + 1)
      else AArgument.Value := ASign * AInt;
      AArgument.Token := etLitFloat;
    end else begin
      APower := Ipwr(10, ADecimals);
      AInt := Trunc(AFloat * APower); //integer part * 10^ADecimals
      AFrac := AFloat * APower - AInt; //fractional part
      if AFrac >= 0.5
      then AArgument.Value := ASign * ipwr(10, - ADecimals) *(AInt + 1)
      else AArgument.Value := ASign * ipwr(10, - ADecimals) * AInt;
      AArgument.Token := etLitFloat;
    end;
  end else begin
    AArgument.Token := etLitInt;
...
var AFloat: Double;
  AExponent: integer;
begin
  if not AArgument.Check([[etLitInt, etLitFloat],[etLitInt]])
  then raise TRtfException.Create(rsUnexpectedParameterType);
  AFloat := AArgument[0].Value;
  AExponent := AArgument[1].Value;
  AArgument.Token := etLitFloat;
  AArgument.Value := IPwr(AFloat, AExponent);
end;
...
begin
  ...
  DecodeDate(AArgument[0].Value, AYear, AMonth, ADay);
  AArgument.Token := etLitString;
  AArgument.Value := AFormatSettings.ShortDayNames[ADay];
end;
...
begin
  ...
  DecodeDate(AArgument[0].Value, AYear, AMonth, ADay);
  AArgument.Token := etLitString;
  AArgument.Value := AFormatSettings.ShortMonthNames[AMonth];
end;
...
begin
  ...
  DecodeDate(AArgument[0].Value, AYear, AMonth, ADay);
  AArgument.Token := etLitString;
  AArgument.Value := AFormatSettings.LongMonthNames[AMonth];
end;
...
begin
  ...
  DecodeDate(AArgument[0].Value, AYear, AMonth, ADay);
  AArgument.Token := etLitString;
  AArgument.Value := Format('%0.4d',[AYear], AFormatSettings);
end;
...
begin
  ...
  DecodeDate(AArgument[0].Value, AYear, AMonth, ADay);
  AArgument.Token := etLitString;
  AArgument.Value := Format('%0.2d',[AMonth], AFormatSettings);
end;
...
begin
  ...
  DecodeDate(AArgument[0].Value, AYear, AMonth, ADay);
  AArgument.Token := etLitString;
  AArgument.Value := Format('%0.2d',[ADay], AFormatSettings);
end;
...
begin
  ...
  then raise TRtfException.Create(rsUnexpectedParameterType);
  if AArgument.Count > 1 then begin
    if not AArgument.Check(1,[etLitString])
    then raise TRtfException.Create(rsUnexpectedParameterType);
    AFormat := AArgument[1].Value;
  end;
  AArgument.Token := etLitString;
  AArgument.Value := FormatDateTime(AFormat, AArgument[0].Value, AFormatSettings);
end;
...
begin
  ...
  then raise TRtfException.Create(rsUnexpectedParameterType);
  AArgument.Token := etLitString;
  AArgument.Value := StrToDate(AArgument[0].Value, AFormatSettings);
end;
...
begin
  ...
  then raise TRtfException.Create(rsUnexpectedParameterType);
  AArgument.Token := etLitString;
  AArgument.Value := StrToTime(AArgument[0].Value, AFormatSettings);
end;
...
begin
  ...
  then raise TRtfException.Create(rsUnexpectedParameterType);
  AArgument.Token := etLitString;
  AArgument.Value := StrToDateTime(AArgument[0].Value, AFormatSettings);
end;
...
var AFormat: string;
  AFloat: Double;
begin
  if not AArgument.Check([[etLitString],[etLitFloat, etLitInt]])
  then raise TRtfException.Create(rsUnexpectedParameterType);
  AFormat := AArgument[0].Value;
  AFloat := AArgument[1].Value;
  AArgument.Token := etLitString;
  AArgument.Value := FormatFloat(AFormat, AFloat, AFormatSettings);
end;
...
begin
  if not AArgument.Check(0,[etLitFalse, etLitTrue])
  then raise TRtfException.Create(rsUnexpectedParameterType);
  if AArgument[0].Token = etLitFalse then begin
    AArgument.Token := etLitString;
...
var ADataset: TRtfDataset;
  AFieldName: string;
begin
  if not AArgument.Check(0,[etFieldName])
  then raise TRtfException.Create(rsUnexpectedParameterType);
  ADataset := Datasets.Find(AArgument[0].Value, AFieldName);
  if not Assigned(ADataset) //There should be at least a table reference.
  then raise TRtfException.Create('Dataset not found');
