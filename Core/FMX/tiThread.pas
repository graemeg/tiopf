...
{$IFDEF IOS}
  TtiThreadList = class(TObjectList<TtiThread>)
{$ELSE}
...
uses
  ...
  ,tiWin32
  {$IFNDEF FPC}
{ TODO : IPK 2012-02-16 Forms commented out in tiThread.pas to get FMX to compile }
...
begin
  result := TtiThread(inherited Items[Index]);
end;
...
begin
  ...
  if Priority < tpNormal then
    Priority := tpNormal;
  {$ELSE}
//  Priority is numeric. What is considered normal?   // Don't know what to do here???
  {$ENDIF}
  FUpdateEvent.Free;
  inherited Destroy;
end;
