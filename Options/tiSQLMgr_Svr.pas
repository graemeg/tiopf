{
  ...
  ToDo:
    ...
    1. Just like Delphi has design time packages (that contain property editors)
      ...
      (linked into tiOPFManager). This would give a tiny performance gain and make
...
begin
  result := ( Visited is TSQLMgrGroup ) and
            (( Visited as TtiObject ).ObjectState = posUpdate ) ;
end;
...
begin
  result := ( Visited is TSQLMgrParam ) and
            (( Visited as TtiObject ).ObjectState = posDelete ) ;
end;
...
begin
  result := ( Visited is TSQLMgrQuery ) and
            (( Visited as TtiObject ).ObjectState = posPK ) ;
end;
...
begin
  {$IFDEF OID_AS_INT64}
    Query.ParamAsInteger['OID'] := ( Visited as TtiObject ).OID ;
  {$ELSE}
    ( Visited as TtiObject ).OID.AssignToTIQuery( Query ) ;
  {$ENDIF}
end;
...

{
  ...
  gTIOPFManager.RegReadPKVisitor( TVisReadGroupPK ) ;
  gTIOPFManager.RegReadPKVisitor( TVisReadQueryPK ) ;

  gTIOPFManager.RegReadVisitor( TVisReadQueryDetail ) ;
  gTIOPFManager.RegReadVisitor( TVisReadParams ) ;

  // The order of these is important:
  // Child nodes must be deleted before parents,
  // Parent nodes must be added before children.
  gTIOPFManager.RegSaveVisitor( TVisDeleteParam ) ;
  gTIOPFManager.RegSaveVisitor( TVisDeleteParamContainer ) ;
  gTIOPFManager.RegSaveVisitor( TVisDeleteQuery ) ;
  gTIOPFManager.RegSaveVisitor( TVisDeleteGroup ) ;
  gTIOPFManager.RegSaveVisitor( TVisUpdateQuery ) ;
  gTIOPFManager.RegSaveVisitor( TVisUpdateParam ) ;
  gTIOPFManager.RegSaveVisitor( TVisUpdateGroup ) ;
  gTIOPFManager.RegSaveVisitor( TVisCreateGroup ) ;
  gTIOPFManager.RegSaveVisitor( TVisCreateQuery ) ;
  gTIOPFManager.RegSaveVisitor( TVisCreateParam ) ;
