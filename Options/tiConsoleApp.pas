...
tiGetConsoleExecutableInfo if you are prepared to do without the protection
it offers
...
{Parameters
  ...
  tiExecConsoleApp does NOT attempt to verify the contents of these files - it
  ...
  will block tiExecConsoleApp from returning until closed. You probably don't
  want to do this.
  ...
  the child process.
  Note that tiExecConsoleApp approximately simulates the behaviour of a real
  ...
  OnNewLine
  The function tiExecConsoleApp does not return until the child process has
...
NOTE CAREFULLY
  In the case of tiExecConsoleApp being unable to run the application it will raise
...
BUG
  tiExecConsoleApp will always fail if the application being spawned is already
...
begin
  ...
  if NewLine then
    AppOutput := AppOutput + cLineEnding + string(LineBuf)
  else
...
begin
  ...
  begin
    BinType:= SCS_32BIT_BINARY;
    SubSyst := IMAGE_SUBSYSTEM_WINDOWS_CUI;
    FmtStr(CommandLine, '"%s" %s', [ApplicationName, Parameters])
  end else if (Ext = '.BAT') or ((Win32Platform = VER_PLATFORM_WIN32_NT) and (Ext = '.CMD')) then
  ...
  begin
    ...
    if Ext = ComExt then
      ...
      tiGetConsoleExecutableInfo(AppNameBuf, BinType, SubSyst);
