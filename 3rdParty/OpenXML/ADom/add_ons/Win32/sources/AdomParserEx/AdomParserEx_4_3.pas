...
//
// The Original Code is "AdomParserEx_4_3.pas".
//
...
{$ELSE}
  AdomCore_4_3,
{$ENDIF}
  dkCodecUtilsWin32, dkWideStringUtils,
  Classes, SysUtils;
...
begin
  ...
  try
    InputSrc := TXmlInputSource.Create(Stream, '', SysId, FBufferSize, AEncoding,
...
begin
  ...
  try
    Result := StreamToDom(StrStream, SysId, AEncoding, InclDecl);
  finally
    StrStream.Free;
  end;
end;
...
begin
  ...
  try
    Result := StreamToDom(WStrStream, SysId, AEncoding, InclDecl);
  finally
    WStrStream.Free;
  end;
end;
