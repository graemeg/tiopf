...
//
// $Id: TBXDkPanels.pas,v 1.62 2003/08/02 04:02:06 Alex Exp $
...
uses
  ...
  TB2Dock, TB2Item, TBX, TBXThemes, ImgList;
...
{$ENDIF}
  ...
  published
    ...
    property Anchors;
    property AutoRange;
//     property AutoScroll; // {TODO: uncomment in the next version }
    property ButtonSize;
    property Color;
    property Constraints;
    property DockSite;
...
begin
  ...
  try
    ...
    // Removed to fix compiler warning
    //DragIndexPos := 0;
...
begin
  if (Message.WParam = 0) and (Color = clNone) then
  begin
    Message.WParam := 1;
...
{  if PaintOptions = cpoTransparent then
...
begin
  ...
  if SmartFocus and CanFocus and Assigned(Parent) and IsActivated(Parent) then
  begin
    FDisableScroll := True;
    SetFocus;
...
begin
  if Enabled and FMouseInControl {and (Mouse.Capture <> Handle)} then
  begin
...
begin
  ...
  begin
    ...
    FillRect(TWMEraseBkgnd(Message).DC, ClientRect, BRUSH);
    DeleteObject(BRUSH);
  end;
  Message.Result := 1;
end;
...
begin
  ...
  begin
    ...
    R := Rect;
    R.Bottom := R.Top + 1;
    Result := DrawText(ACanvas.Handle, PChar(Text), Length(Text), R, Flags);
  end
...
const
  ...
  ShowAccelChars: array [Boolean] of Integer = (0, DT_NOPREFIX);
var
  R, R2: TRect;
  DrawStyle: Longint;
...
begin
  ...
  begin
    ...
    DrawStyle := DT_EXPANDTABS or WordWraps[Wrapping] or
      Alignments[GetTextAlignment] or ShowAccelChars[ShowAccelChar];
    Brush.Style := bsClear;
...
begin
  if csDesigning in ComponentState then
  begin
    Canvas.Brush.Color := MixColors(clBtnFace, clBtnShadow, 128);
    Canvas.FrameRect(Rect(0, 0, ClientWidth, ClientHeight));
  end;
end;
...
const
  EnabledState: array [Boolean] of Integer = (GS_DISABLED, 0);
  StateFlags: array [TCheckBoxState] of Integer = (0, GS_CHECKED, GS_MIXED);
  HotState: array [Boolean] of Integer = (0, GS_HOT);
  PushedState: array [Boolean] of Integer = (0, GS_PUSHED);
  FocusedState: array [Boolean] of Integer = (0, GS_FOCUSED);
var
  Rect: TRect;
  Sz, Flags: Integer;
begin
  ...
  begin
    ...
    CurrentTheme.PaintGlyph(Canvas, Rect, GK_CHECKBOX, Flags);
  end;
end;
...
const
  EnabledState: array [Boolean] of Integer = (GS_DISABLED, 0);
  CheckedState: array [Boolean] of Integer = (0, GS_CHECKED);
  HotState: array [Boolean] of Integer = (0, GS_HOT);
  PushedState: array [Boolean] of Integer = (0, GS_PUSHED);
  FocusedState: array [Boolean] of Integer = (0, GS_FOCUSED);
var
...
begin
  ...
  begin
    ...
    if Enabled then Flags := Flags or CheckedState[Checked] or
      HotState[MouseInControl] or PushedState[Pushed and MouseInControl] or FocusedState[Focused];
    CurrentTheme.PaintGlyph(Canvas, Rect, GK_RADIOBUTTON, Flags);
  end;
end;
