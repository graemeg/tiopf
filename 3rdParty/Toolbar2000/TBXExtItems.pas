...
//
// $Id: TBXExtItems.pas,v 1.55 2003/07/28 05:41:01 Alex Exp $
...
type
  ...
  protected
    ...
    procedure DoChange(Edit: TEdit; const AText: string); virtual;
  ...
  protected
    ...
    procedure HandleEditChange(Edit: TEdit; const AText: string); virtual;
  ...
  protected
    ...
    procedure ClickUp;
    procedure ClickDown;
    procedure DoChange(Edit: TEdit; const AText: string); override;
...
begin
  ...
  begin
    ...
    FontSettings.Apply(EditControl.Font);
    inherited;
...
procedure TTBXEditItem.DoChange(Edit: TEdit; const AText: string);
begin
...
//    FillChar(EventData, SizeOf(EventData), 0);
    ...
    try
      ...
      AView.SetCapture;
      SaveText := Text;
      TTBXEditItemViewer(V).KeyDown(Key, []);
...
begin
  ...
  if Item is TTBXEditItem then
    TTBXEditItem(Item).FontSettings.Apply(Canvas.Font);
  DC := Canvas.Handle;
  TextHeight := GetTextHeight(DC);
  Inc(TextHeight); // just looks better this way
  ...
  begin
    ...
    begin
      if EditCaptionIndented then Inc(AWidth, GetPopupMargin(Self) + MenuImageTextSpace)
      else Inc(AWidth, MenuLeftCaptionMargin);
      if Length(Item.EditCaption) > 0 then
        Inc(AWidth, GetTextWidth(DC, Item.EditCaption, True) +
        EditMenuMidWidth + MenuRightCaptionMargin);
    end;
    MinHeight := AHeight + EditTextMarginVert * 2 + EditFrameWidth * 2;
    if not IsToolbarStyle then Inc(AHeight, DivRoundUp(AHeight, 4))
    else Inc(AHeight, EditFrameWidth * 2);
...
begin
  HandleEditChange((Sender as TEdit), (Sender as TEdit).Text);
end;

procedure TTBXEditItemViewer.HandleEditChange(Edit: TEdit; const AText: string);
begin
  TTBXEditItem(Item).DoChange(Edit, AText);
end;
...
begin
  ...
  try
    ...
    if Length(Item.EditCaption) > 0 then W := GetTextWidth(DC, Item.EditCaption, True) + MenuRightCaptionMargin
    else W := 0;

    R := BoundsRect;
    if not IsToolbarStyle then
    begin
      if EditCaptionIndented then
        Inc(R.Left, GetPopupMargin(Self) + MenuLeftCaptionMargin)
      else
        if W > 0 then Inc(R.Left, MenuLeftCaptionMargin + EditMenuMidWidth);
    end;
    Inc(R.Left, W);
    B := CurrentTheme.EditFrameWidth - TB2K_EDIT_BORDER;
...
begin
  ...
  begin
    ...
    begin
      Canvas.Font := TTBViewAccess(View).GetFont;
      if Item is TTBXEditItem then TTBXEditItem(Item).FontSettings.Apply(Canvas.Font);
      W := GetTextWidth(Canvas.Handle, S, True);
    end
    else W := 0;
    ...
    begin
      { OfficeXP - style }
      M := GetPopupMargin(Self) + MenuLeftCaptionMargin + MenuMarginHorz + 1;
      R.Right := R.Left + M + MenuMarginHorz;
      if W > 0 then Inc(R.Right, W + MenuRightCaptionMargin);
      CurrentTheme.PaintMenuItemFrame(Canvas, R, ItemInfo);
      Dec(R.Right, MenuRightCaptionMargin);
    end
    else if W > 0 then
    begin
      { Default tb2k-like style }
      M := MenuLeftCaptionMargin;
      R.Right := R.Left + M + W + MenuRightCaptionMargin;
      CurrentTheme.PaintMenuItemFrame(Canvas, R, ItemInfo);
    end
    else M := 0;

    Inc(R.Left, M);
    CurrentTheme.PaintCaption(Canvas, R, ItemInfo, S,
      DT_SINGLELINE or DT_LEFT or DT_VCENTER, False);
    R.Left := R.Right;
    R.Right := ClientAreaRect.Right;

    if EditCaptionIndented then Dec(R.Right, MenuMarginHorz)
    else if W > 0 then Inc(R.Left, EditMenuMidWidth);
  end;
  ...
  SaveFont := SelectObject(Canvas.Handle, ToolbarFont.Handle);
  SetBkMode(Canvas.Handle, TRANSPARENT);
  if Item.Text <> '' then
  begin
    S := Item.Text;
    if TTBXEditItem(Item).PasswordChar <> #0 then
      S := StringOfChar(TTBXEditItem(Item).PasswordChar, Length(S));
    SetBkColor(Canvas.Handle, GetSysColor(FillColors[Item.Enabled]));

    Canvas.Font.Color := TextColors[Item.Enabled];
    if Item is TTBXEditItem then TTBXEditItem(Item).FontSettings.Apply(Canvas.Font);
    DrawText(Canvas.Handle, PChar(S), Length(S), R, DT_SINGLELINE or
      DT_NOPREFIX or Alignments[Item.Alignment]);
  end;
  SelectObject(Canvas.Handle, SaveFont);
  Inc(R.Right, 1);
end;
...
begin
  inherited;
  ItemStyle := ItemStyle + [tbisSubmenu, tbisCombo, tbisSubitemsEditable] - [tbisDontSelectFirst];
...
begin
  ...
  else Result := GetSystemMetrics(SM_CXMENUCHECK);
end;
...
begin
  inherited;
  SaveText := Viewer.EditControl.Text;
end;

procedure TTBXComboBoxItem.DoChange(Edit: TEdit; const AText: string);
var
  I: Integer;
  S: string;
begin
  inherited;
  CacheValid := False;
  if IsChanging or (csLoading in ComponentState) then Exit;
  IsChanging := True;
  try
    S := AText;
    if Length(S) > 0 then
      for I := 0 to FList.Strings.Count - 1 do
        if StartsText(S, FList.Strings[I]) then
        begin
          FList.MakeVisible(I);
          if AutoComplete and (Edit <> nil) and
            (Length(S) > Length(SaveText)) and StartsText(SaveText, S) then
          begin
            Edit.Text := FList.Strings[I];
            Edit.SelStart := Length(S);
            Edit.SelLength := Length(FList.Strings[I]) - Length(S);
            S := FList.Strings[I];
          end;
          if CompareText(S, FList.Strings[I]) = 0 then FList.ItemIndex := I
          else FList.ItemIndex := -1;
          Exit;
        end;
    FList.ItemIndex := -1;
  finally
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        Text := FList.Strings[FList.ItemIndex];
        DoChange(nil, Text);
      end;
    finally
      IsChanging := False;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Inc(AWidth, GetTextWidth(DC, S, True));
      end
      else
      begin
        ...
        Inc(AHeight, GetTextWidth(DC, S, True));
...
begin
  ...
  Fmt := DT_SINGLELINE or DT_CENTER or DT_VCENTER;
  R := ClientAreaRect;
...
procedure TTBXCustomSpinEditItem.DoChange(Edit: TEdit; const AText: string);
var
  V: Extended;
begin
  if not FChanging then
  begin
    if ParseValue(AText, V) then
    begin
      V := Self.CheckValue(V);
      if V <> FLastGoodValue then
      begin
        FLastGoodValue := V;
        DoValueChange(V);
      end;
...
var
  ...
  begin
    ...
    begin
      ...
      if (ValueType = evtFloat) and (P^ = FormatSettings.DecimalSeparator) then
      begin
        Inc(P);
        PStart := P;
        R := R + GetInt2 / IntPower(10, P - PStart);
        Count2 := P - PStart;
      end
      else Count2 := 0;
    ...
    begin
      ...
      begin
        ...
        else if P^ in ['a'..'f'] then Inc(IR, Integer(P^) - Integer('a'))
        else Inc(IR, Integer(P^) - Integer('A'));
        Inc(P);
      end;
      R := IR;
    end;
...
begin
  ...
  if Message.Msg = WM_CHAR then
    case TWMChar(Message).CharCode of
      VK_TAB:
        begin
          Item.Value := Item.FLastGoodValue;
