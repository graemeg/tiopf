...
//
// $Id: TBX.pas,v 1.100 2003/07/28 05:41:01 Alex Exp $
...
begin
  ...
  begin
    ...
    begin
      ...
      FillRectEx2(DC, R, GetEffectiveColor(Parent))
    else
    begin
      ...
      else FillRectEx2(DC, R, GetEffectiveColor(Parent));
    end;
  end;
end;
...
procedure AdjustFontSize(AFont: TFont; AFontSize: TTBXFontSize);
begin
  case AFontSize of
    tbxfSmaller: AFont.Size := Abs(AFont.Size) * 7 div 8;
    tbxfLarger: AFont.Size := Abs(AFont.Size) * 9 div 8;
  end;
end;
...
begin
  ...
  M2 := CurrentTheme.MenuMarginVert * 2;
...
begin
  ...
  if not Boolean(StateFlags and ISF_MENUCOLOR) then
  begin
...
begin
  ...
  CurrentTheme.GetMargins(CMarginIDs[ToolbarStyle], Margins);
  ...
  begin
    ...
    begin
      ...
      begin
        if Wide then Inc(AWidth, ImgSize.CX + 1)
        else Inc(AWidth, ImgSize.CX);
    ...
    begin
      AWidth := TextSize.CX + MenuMarginHorz * 2;
      AHeight := TextSize.CY + MenuMarginVert * 2;
      GetTextMetrics(Canvas.Handle, TextMetric);
      Inc(AHeight, TextMetric.tmExternalLeading);
      if ImgSize.CX = 0 then
      begin
        ImgSize.CX := 16;
        ImgSize.CY := 16;
      end;
      H := ImgSize.CY + MenuMarginVert * 2;
...
//  if not ToolbarStyle then ItemInfo.PopupMargin := GetPopupMargin(Self);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        ComboRect.Top := R.Top + ((R.Bottom - R.Top) - ImgSize.CY - TextSize.CY) div 2;
        ComboRect.Bottom := R.Top + ImgSize.CY;
      end;
    end
  ...
  begin
    CaptionRect := R;
    PaintMenuItem(Canvas, CaptionRect, ItemInfo);
    M := CurrentTheme.MenuMarginHorz;
    Inc(CaptionRect.Left, ItemInfo.PopupMargin + M);
    Dec(CaptionRect.Right, M);
    Inc(R.Left, M);
    Dec(R.Right, M);
  end;
  ...
  begin
    ...
    begin
      ...
      Inc(CaptionRect.Left, MenuImageTextSpace + MenuLeftCaptionMargin);
      GetTextMetrics(Canvas.Handle, TextMetrics);
      with TextMetrics, CaptionRect do
        if (Bottom - Top) - (tmHeight + tmExternalLeading) = MenuMarginVert then Dec(Bottom);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (Bottom - Top) - (tmHeight + tmExternalLeading) = CurrentTheme.MenuMarginVert then
          Dec(Bottom);
  ...
  begin
    ...
    begin
      ...
      else ImageRect.Bottom := ImageRect.Top + ImgSize.CY + 2;
    end
    ...
    begin
      Left := Left + ((Right - Left) - ImgSize.CX) div 2;
      Top := Top + ((Bottom - Top) - ImgSize.CY) div 2;
      Right := Left + CX;
      Bottom := Top + CY;
...
begin
  ...
  with ItemInfo do
    HorzLine := (IsVertical xor LineSep) or View.IsPopup or
      (((ViewType and VT_POPUP) = VT_POPUP) and
      ((ViewType and PVT_CHEVRONMENU) <> PVT_CHEVRONMENU));
...
begin
  ...
  PR := Rect(0, 0, 0, 0);
  if (View <> nil) and (View.ParentView <> nil) and
    (CurrentTheme.GetPopupShadowType = PST_OFFICEXP) then
  begin
...
begin
  ...
  try
    ...
    try
      ...
      else if CurrentTheme.SolidToolbars then
      begin
        ACanvas.Brush.Color := EffectiveColor;
        ACanvas.FillRect(R);
...
begin
  ...
  CR := ClientRect;
  if Color = clNone then
    if Docked and (TDockAccess(CurrentDock).UsingBackground) and CurrentDock.BackgroundOnToolbars then
    begin
...
begin
  ...
  try
    ...
    try
      ...
      Canvas.Brush.Color := GetEffectiveColor(DockWindow);
...
begin
  ...
  else Message.WParam := {$IFDEF DELPHIXE2ORABOVE}UIntPtr(clBtnFace){$ELSE}clBtnFace{$ENDIF};
  Message.Result := 1;
end;
